...
class TestTau6X2LStateDocs:
    def test_changelog_records_tau6x2l(self):
        txt = (REPO / "dev" / "CHANGELOG.md").read_text(encoding="utf-8")
        assert "τ.6.x.2.l" in txt

    def test_session_state_mentions_tau6x2l(self):
        txt = (REPO / "dev" / "SESSION_STATE.md").read_text(encoding="utf-8")
        assert "τ.6.x.2.l" in txt

    def test_in_flight_mentions_tau6x2l(self):
        txt = (REPO / "dev" / "IN_FLIGHT.md").read_text(encoding="utf-8")
        assert "τ.6.x.2.l" in txt

    def test_plan_ledger_records_tau6x2l(self):
