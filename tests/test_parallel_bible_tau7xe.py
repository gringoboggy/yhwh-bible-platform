...
class TestTau7XEGeezTewahedoPreserved:
    ...
    def test_geez_tewahedo_deu_py_not_created(self):
        assert not (GEEZ_TEWAHEDO / "deu.py").exists(), (
            "geez-tewahedo/deu.py must NOT be created at τ.7.x.e; Geʽez Deuteronomy is τ.6.x.2.e under D4-c sequencing"
        )
...
class TestTau7XEStateDocs:
    """SESSION_STATE, IN_FLIGHT, CHANGELOG, PLAN all reference τ.7.x.e."""

    def test_session_state_mentions_tau7xe(self):
        txt = (REPO / "dev" / "SESSION_STATE.md").read_text(encoding="utf-8")
        assert "τ.7.x.e" in txt

    def test_in_flight_mentions_tau7xe(self):
        txt = (REPO / "dev" / "IN_FLIGHT.md").read_text(encoding="utf-8")
        assert "τ.7.x.e" in txt

    def test_changelog_records_tau7xe_entry(self):
        txt = (REPO / "dev" / "CHANGELOG.md").read_text(encoding="utf-8")
        assert "τ.7.x.e" in txt

    def test_plan_ledger_records_tau7xe(self):
