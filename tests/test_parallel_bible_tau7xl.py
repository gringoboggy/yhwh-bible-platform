...
class TestTau7XLMDeuterocanonArcAndPriorPins:
    ...
    def test_geez_jdt_est_ingested_durable(self):
        """MIGRATED TWICE per memory `feedback_share_pin_pattern` +
        the τ.6.x.2.j/k precedent. Originally `test_geez_jdt_est_not_
        created` (BOTH must NOT exist, D4-c deferral); at τ.6.x.2.l
        the jdt half flipped to "must EXIST"; at τ.6.x.2.m the est
        half flips too. Now a DURABLE positive invariant: the Geʽez
        Judith (τ.6.x.2.l) + Esther (τ.6.x.2.m) catchup mirrored this
        Amharic τ.7.x.l/m pair — both present at ocr-tier3; never
        asserts what has NOT shipped (no anti-monotonic frontier)."""
        assert (GEEZ_TEWAHEDO / "jdt.py").is_file(), "Geʽez jdt.py must EXIST after the τ.6.x.2.l catchup ship"
        assert (GEEZ_TEWAHEDO / "est.py").is_file(), "Geʽez est.py must EXIST after the τ.6.x.2.m catchup ship"
...
class TestTau7XLMStateDocs:
    ...
    def test_phase_recorded_in_changelog(self):
        ...
        assert_phase_recorded("τ.7.x.l", "τ.7.x.m")
