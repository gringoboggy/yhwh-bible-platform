...
class TestTau6X0cClosedArcInvariantPreservation:
    ...
    def test_geez_tewahedo_still_gen_only(self):
        ...
        assert files == ["gen.py"], (
            f"τ.6.x.0c contract: geez-tewahedo must remain at τ.6 seed (gen.py only); got {files}"
        )
    ...
    def test_amharic_tewahedo_still_gen_only(self):
        ...
        files = sorted(p.name for p in slot.iterdir() if p.suffix == ".py")
        assert files == ["gen.py"], (
            f"τ.6.x.0c contract: amharic-tewahedo must remain at Π.0 seed (gen.py only); got {files}"
        )
