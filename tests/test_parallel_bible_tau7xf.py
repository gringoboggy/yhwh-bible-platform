...
class TestTau7XFStateDocs:
    """SESSION_STATE, IN_FLIGHT, CHANGELOG, PLAN all reference τ.7.x.f."""

    def test_session_state_mentions_tau7xf(self):
        txt = (REPO / "dev" / "SESSION_STATE.md").read_text(encoding="utf-8")
        assert "τ.7.x.f" in txt

    def test_in_flight_mentions_tau7xf(self):
        txt = (REPO / "dev" / "IN_FLIGHT.md").read_text(encoding="utf-8")
        assert "τ.7.x.f" in txt

    def test_changelog_records_tau7xf_entry(self):
        txt = (REPO / "dev" / "CHANGELOG.md").read_text(encoding="utf-8")
        assert "τ.7.x.f" in txt

    def test_plan_ledger_records_tau7xf(self):
