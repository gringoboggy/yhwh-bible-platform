...
class TestAnthropicXrefClient:
    ...
    def test_default_model_uses_alias_not_dated_id(self):
        ...
        # No date suffix
        assert not any(c.isdigit() and i > len("claude-haiku-4-5")
                       for i, c in enumerate(self.src.DEFAULT_AI_XREF_MODEL))
    ...
    def test_output_schema_locks_proposal_shape(self):
        ...
        assert {"target_book", "target_chapter", "target_verse",
                "kind_subclass", "reasoning", "confidence"} <= required
...
class TestLauncherShouldRunFirstRunMigration:
    ...
    def test_returns_true_when_frozen_and_marker_missing(
        ...
        monkeypatch.setattr(sys, "frozen", True, raising=False)
        monkeypatch.setattr(paths_mod, "user_data_root",
                            lambda: tmp_path)
...
class TestLauncherBuildUrl:
    ...
    def test_localhost_address(self):
        assert self.mod.build_url("127.0.0.1", 8765) == \
            "http://127.0.0.1:8765/"
    ...
    def test_displays_localhost_for_zero(self):
        ...
        assert self.mod.build_url("0.0.0.0", 8765) == \
            "http://localhost:8765/"

    def test_arbitrary_port(self):
        assert self.mod.build_url("127.0.0.1", 9999) == \
            "http://127.0.0.1:9999/"
...
class TestLauncherBootstrap:
    ...
    def test_default_composes_migrate_module(self, monkeypatch):
        ...
        monkeypatch.setattr(mm, "_src_content",
                            lambda: Path("/nonexistent/src"))
        monkeypatch.setattr(mm, "_dst_content",
                            lambda: Path("/nonexistent/dst"))
...
class TestLauncherStartServer:
    ...
    def test_uses_injected_factory(self):
        ...
        result = self.mod.start_server(
            "127.0.0.1", 8765, server_factory=fake_factory,
        )
...
class TestLauncherMain:
    ...
    def test_reports_port_fallback(self, capsys, monkeypatch):
        ...
        monkeypatch.setattr(self.mod, "find_free_port",
                            lambda port, host: port + 1)
        rc = self.mod.main(
...
class TestDesktopShellSelectShellMode:
    ...
    def test_force_native_overrides_everything(self):
        assert self.mod.select_shell_mode(
            frozen=False, available=False, force="native",
        ) == "native"

    def test_force_browser_overrides_everything(self):
        assert self.mod.select_shell_mode(
            frozen=True, available=True, force="browser",
        ) == "browser"
    ...
    def test_auto_dev_picks_browser(self):
        ...
        assert self.mod.select_shell_mode(
            frozen=False, available=True,
        ) == "browser"

    def test_auto_frozen_with_pywebview_picks_native(self):
        assert self.mod.select_shell_mode(
            frozen=True, available=True,
        ) == "native"
    ...
    def test_unknown_force_value_falls_through_to_auto(self):
        ...
        assert self.mod.select_shell_mode(
            frozen=False, available=True, force="auto",
        ) == "browser"
...
class TestDesktopShellWindowConfig:
    ...
    def test_overrides_apply(self):
        cfg = self.mod.window_config(
            "http://x/", title="Custom", width=800, height=600,
            min_size=(400, 300), resizable=False,
        )
...
class TestDesktopShellOpenInNativeShell:
    ...
    def test_passes_debug_flag(self):
        ...
        self.mod.open_in_native_shell(
            "http://x/", webview_module=FakeWebview, debug=True,
        )
    ...
    def test_raises_when_pywebview_missing(self, monkeypatch):
        ...
        assert "pywebview" in str(exc.value).lower() or \
            "PyWebView" in str(exc.value)
...
class TestPsi181MatrixPerChapterField:
    ...
    def test_per_chapter_keyed_by_edition(self):
        ...
        assert set(self.matrix.per_chapter.keys()) == set(
            self.matrix.per_book.keys()
        )
    ...
    def test_per_chapter_book_subset_matches_per_book(self):
        ...
        for ed_id, by_kind in self.matrix.per_chapter.items():
            for kind, by_book in by_kind.items():
                ...
                for book in by_book:
                    assert book in pb, (
                        f"{ed_id}/{kind}/{book}: chapter detail "
                        f"without per_book entry"
                    )
        ...
        for ed_id, by_kind in self.matrix.per_chapter.items():
            for kind, by_book in by_kind.items():
                ...
                for book, by_ch in by_book.items():
                    ...
                    assert summed == pb[book], (
                        f"{ed_id}/{kind}/{book}: chapter sum="
                        f"{summed} but per_book={pb[book]}"
                    )
    ...
    def test_per_chapter_keys_are_ints(self):
        ...
        for ed_id, by_kind in self.matrix.per_chapter.items():
            for kind, by_book in by_kind.items():
                for book, by_ch in by_book.items():
                    for ch_key in by_ch:
                        assert isinstance(ch_key, int), (
                            f"{ed_id}/{kind}/{book}: chapter key "
                            f"{ch_key!r} is not int"
                        )
            ...
            for kind, by_book in by_kind.items():
                for book, by_ch in by_book.items():
                    for ch, count in by_ch.items():
                        assert count > 0, (
                            f"{ed_id}/{kind}/{book}/{ch}: stored "
                            f"zero count (should be absent)"
                        )
...
class TestPsi181ApiMatrixPerChapterSurface:
    ...
    def test_response_includes_per_chapter(self):
        for ed_id, ed_data in self.api["matrix"].items():
            assert "per_chapter" in ed_data, (
                f"{ed_id}: missing per_chapter key"
            )
            ...
            for book in ed_data["canon_book_order"]:
                if book in counts:
                    assert counts[book] > 0, (
                        f"{ed_id}/{book}: ch_count is 0 or negative"
                    )
        ...
        for ed_id, ed_data in self.api["matrix"].items():
            ...
            for kind, books in api_pc.items():
                for book, by_ch in books.items():
                    ...
                    assert api_sum == mod_sum, (
                        f"{ed_id}/{kind}/{book}: API chapter-sum"
                        f"={api_sum} but module={mod_sum}"
                    )
        ...
        for ed_id, ed_data in self.api["matrix"].items():
            for book, ch_count in ed_data["book_chapter_counts"].items():
                assert yaml_counts.get(book) == ch_count, (
                    f"{ed_id}/{book}: API ch_count={ch_count} but "
                    f"books.yaml={yaml_counts.get(book)}"
                )
...
class TestPsi181MatrixHtmlChapterDrilldown:
    ...
    def test_drilldown_css_suppresses_global_arrow(self):
        ...
        assert (
            "details.psi181-drilldown > summary::before"
            in self.html
        )
...
class TestPsi15EditorConsoleHeaderNavSubstitution:
    ...
    def test_other_consoles_marked_text_blue_600(self):
        ...
        for name, expected in cases.items():
            assert expected in self.htmls[name], (
                f"{name}: missing other-link with text-blue-600"
            )
    ...
    def test_canonical_label_symbol_matrix_present(self):
        ...
        for name, html in self.htmls.items():
            ...
            assert ">symbol matrix<" in html, (
                f"{name}: missing canonical 'symbol matrix' label"
            )
