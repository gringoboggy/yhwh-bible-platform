...
class TestTau7XDGeezTewahedoPreserved:
    ...
    def test_geez_tewahedo_num_py_ingested_at_tau6x2d(self):
        """MIGRATED at τ.6.x.2.a-h batch ship-time (2026-05-15):
        originally asserted geez-tewahedo/num.py does NOT exist
        until τ.6.x.2.d ships. The τ.6.x.2.a-h batch ship
        CREATED this file at ocr-tier3 quality (per D4-c catchup arc).
        Durable assertion is now: geez-tewahedo/num.py EXISTS at
        ocr-tier3 ingest scale; per-file content pinned in
        test_parallel_bible_tau6x2_geez_arc.py."""
        import ast

        path = GEEZ_TEWAHEDO / "num.py"
        assert path.is_file(), "geez-tewahedo/num.py must exist post-τ.6.x.2.d (τ.6.x.2.a-h batch ship)"
        ...
        # τ.6.x.2.d empirical at ship: 830 verses; floor 780 guards
        # against regression while permitting parser refinement.
        assert len(verses) >= 780, (
            f"geez-tewahedo/num.py must be at ocr-tier3 scale post-τ.6.x.2.d; "
            f"got {len(verses)} verses (<780 indicates regression)"
        )
