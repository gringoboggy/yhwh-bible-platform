...
def build_plan(
    ...
    *,
    ...
    if not target.is_file():
        return ScaffoldPlan(
            name=name, title=title, route=route, constant_name=constant,
            description=description, target_file=target,
            will_create_constant=False, will_register_route=False,
            will_inject_nav=0, will_update_linter=False,
    ...
    if target == WEB_PY and TEMPLATES_DIR.is_dir():
        existing = list(set(existing) | set(
            bulk_inject.list_constants(TEMPLATES_DIR)
        ))
    if constant in existing:
        return ScaffoldPlan(
            ...
            will_update_session_state=False,
            skipped_reason=(
                f"constant {constant} already exists; "
                f"refusing to overwrite (idempotent guard)"
            ),
        )
    ...
    return ScaffoldPlan(
        name=name, title=title, route=route, constant_name=constant,
        description=description, target_file=target,
        will_create_constant=True, will_register_route=True,
...
def render_constant(plan: ScaffoldPlan, *, existing_consoles: list[str]) -> str:
    ...
    for href, label in nav_links:
        nav_parts.append(
            f'    <a href="{href}" class="text-blue-600 hover:underline">{label}</a>'
        )
    nav_parts.append(
        f'    <a href="{self_link[0]}" class="font-semibold">{self_link[1]}</a>'
    )
    nav_block = "\n".join(nav_parts)
...
def apply_plan(
    ...
    *,
    ...
    if TEMPLATES_DIR.is_dir() and target == WEB_PY:
        existing = sorted(set(existing) | set(
            bulk_inject.list_constants(TEMPLATES_DIR)
        ))
    ...
    if use_templates_dir:
        ...
        import_line = (
            f"from scripts.templates.{plan.name.replace('-', '_')} "
            f"import {plan.constant_name}\n"
        )
    ...
    nav_link = (
        f'<a href="{plan.route}" class="text-blue-600 hover:underline">'
        f'{plan.name}</a>\n    '
    )
...
def format_plan_report(plan: ScaffoldPlan) -> str:
    ...
    if plan.skipped_reason:
        ...
        lines.append(f"    [{('x' if plan.will_create_constant else ' ')}] create {plan.constant_name} constant in {constant_dest}")
        lines.append(f"    [{('x' if plan.will_register_route else ' ')}] register route {plan.route}")
        lines.append(f"    [{('x' if plan.will_inject_nav > 0 else ' ')}] inject nav link into {plan.will_inject_nav} existing console(s)")
        lines.append(f"    [{('x' if plan.will_update_linter else ' ')}] update lint_rules.py route_for_constant table")
        lines.append(f"    [{('x' if plan.will_update_session_state else ' ')}] update SESSION_STATE.md consoles inventory")
    return "\n".join(lines)
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("name",
                        help="console name (lowercase, e.g. 'ops', 'dashboard')")
    parser.add_argument("--title", required=True,
                        help='display title (e.g. "Operator Dashboard")')
    parser.add_argument("--route", default=None,
                        help="URL route (default: /<name>)")
    parser.add_argument("--description", default=None,
                        help="optional one-line description below the title")
    parser.add_argument("--target", type=Path, default=None,
                        help="target Python file (default: scripts/web.py)")
    ...
    try:
        plan = build_plan(
            ...
            route=args.route, description=args.description,
