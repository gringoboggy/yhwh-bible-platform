...
def run_hebrew_for_book(book: str, *, min_confidence: float = 0.7) -> dict:
    ...
    if book in NT_BOOKS:
        return {"book": book, "skipped": True, "reason": "NT (no Hebrew)",
...
def main() -> int:
    ...
    p.add_argument("--books",
                    help="comma-separated list of OT books (default: all)")
    ...
    print(f"Running HebrewWordDetector across {len(books)} OT books "
