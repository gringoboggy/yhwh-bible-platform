"""γ.1 — /hebrew console template (Hebrew interlinear UI).

Surfaces the Strong's Hebrew lexicon as a searchable admin console.
Type a Strong's number (H1, 1, H0001 — all work) and see the
lemma + transliteration + pronunciation + derivation + definition.

Composes the full ζ-foundation:
    - ζ.1 surfaces (theme-bg-page, theme-bg-surface, theme-text)
    - ζ.4 typography (theme-text-2xl heading, theme-font-mono Hebrew)
    - ζ.5 icons (info icon next to attribution)
    - ζ.6 toasts (fetch error → ebibleToast('...', 'error'))
    - ζ.7 skeleton (no — fetches are fast; not needed here)
    - ζ.8 Cmd+K palette (auto-via the marker)

Foundation for γ.2 Greek (same pattern, swap /api/hebrew → /api/greek)
and a future γ.1.x that wires Hebrew data into build_edition.py for
buyer-facing EPUB Hebrew interlinear rendering.
"""
...
<main class="max-w-3xl mx-auto px-4 py-6">
  <h1 class="theme-text-2xl theme-weight-semibold mb-2">Hebrew Interlinear</h1>
  <p class="theme-text-sm theme-text-muted mb-6">
    Look up a Strong's Hebrew number — covers all 8,674 entries.
    Type <code class="theme-font-mono">H1</code> for the first
    entry (<span dir="rtl" class="theme-font-mono">אָב</span>, "father") or
    <code class="theme-font-mono">H7225</code> for Genesis 1:1's first
    word.
  </p>
  ...
  <p class="theme-text-xs theme-text-muted mt-8">
    Source: Strong's <em>A Concise Dictionary of the Words in the Hebrew Bible</em>
    (James Strong, 1890). Public domain.
  </p>
</main>
...
function renderEntry(data) {
  ...
  // γ.1 — render via DOM nodes (not innerHTML interpolation) so any
  // exotic-codepoint Hebrew content stays XSS-safe by construction.
  root.innerHTML = '';

  const card = document.createElement('section');
  card.className = 'theme-bg-surface theme-border border rounded-lg p-6 space-y-4';

  // Header: number + lemma + transliteration
  ...
  lemma.className = 'hebrew-lemma theme-weight-bold';
  ...
  pron.textContent = data.pron ? '/' + data.pron + '/' : '';
...
async function lookup(rawNum) {
  ...
  try {
    const r = await fetch('/api/hebrew/' + encodeURIComponent(num));
...
// Auto-lookup on URL hash (e.g. /hebrew#H7225) for shareable links.
...
# γ.1: design-system substitution (cross-link nav + theme markers).
HEBREW_HTML = apply_design_system(HEBREW_HTML, "/hebrew")
