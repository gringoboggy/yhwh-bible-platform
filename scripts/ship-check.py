...
def run(label: str, cmd: list[str], verbose: bool, *, allow_warn_exit_code: bool = False) -> tuple[bool, str]:
    ...
    passed = (r.returncode == 0) or (allow_warn_exit_code and r.returncode == 1
                                      and "1354/1354 paired" in r.stdout)
...
def main() -> None:
    ...
    p.add_argument("--verbose", "-v", action="store_true",
                   help="show full sub-tool output")
    p.add_argument("--skip-onix", action="store_true",
                   help="skip ONIX TODO_* check (e.g., when intentionally pre-submission)")
    p.add_argument("--retail", action="store_true",
                   help="add EPUB-validation gate (epubcheck) — required pre-submission")
    p.add_argument("--editions-dir", type=Path,
                   help="directory of built EPUBs to validate (with --retail)")
    args = p.parse_args()

    print(f"\n{BOLD}ship-check{RESET}  {DIM}pre-flight integrity gate"
          f"{'  (retail mode)' if args.retail else ''}{RESET}\n")
    ...
    ok, summary = run("xref-targets-dry-run",
    ...
    for label, ok, summary in results:
        ...
        clean = "".join(part for part in summary.split("\033[")
                        if not part or part[0].isdigit() and "m" in part[:5])
    ...
    if pass_count == len(results):
        ...
        print(f"  {RED}{BOLD}✗ {failed} CHECK(S) FAILED{RESET} "
              f"{DIM}({pass_count}/{len(results)} passed){RESET}\n")
