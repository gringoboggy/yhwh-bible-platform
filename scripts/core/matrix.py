...
Inputs (all read via cached config loaders):
    ...
    content/notes/*.py      — actual notes (91,720 as of 2026-06-17; snapshot, re-verify via trace_matrix.py — not a pin)
...
def _enabled_kinds_for_edition(edition: dict, all_kinds: Iterable[dict]) -> set[str]:
    """Final set of kind codes that ship in this edition.

    Thin wrapper over the canonical
    :func:`scripts.core.config.enabled_kind_codes` so the matrix count grid,
    the EPUB build, and ``config._kinds_in_edition`` all resolve "which kinds
    ship" identically. They used to drift — this function lacked the phase
    gate and so the matrix over-counted vs. what actually builds. ``all_kinds``
    is accepted as any iterable for back-compat with existing callers.
    """
    return config.enabled_kind_codes(edition, list(all_kinds))


# Re-exported from config (the canonical home, alongside enabled_kind_codes)
# so existing `from scripts.core.matrix import AI_DRAFTED_KINDS` imports
# (promote.py, tests) keep working.
AI_DRAFTED_KINDS = config.AI_DRAFTED_KINDS
