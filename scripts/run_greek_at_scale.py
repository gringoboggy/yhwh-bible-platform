...
Thin CLI over ``at_scale_base.run_word_detector_*`` (v0.1.0 STAGE A): the shared
per-book loop + ``main()`` live in ``scripts/core/at_scale_base.py``; this file
supplies the GreekWordDetector instance + the NT-scope labels.
...
# Greek lexicon is NT-only: a book is in scope iff it IS a NT book.
_OUT_OF_SCOPE_REASON = "OT (no NT Greek)"
...
def main() -> int:
    return run_word_detector_main(
        ...
        detector_label="GreekWordDetector",
        scope_label="NT",
        skipped_scope_label="OT",
