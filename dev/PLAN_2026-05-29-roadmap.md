...
| identity-docs | ✅ LICENSE + COPYRIGHT stance DECIDED (2026-06-04, user) = **all-rights-reserved / source-available — NOT CC0** (the old "CC0" was stale + contradicted the website); name = **Bogdan Zorlescu**, contact = `gringo.boggy@yhwhyaway.com` filled, note count 67,713→91,733. **Tail:** verify the de-commercialize pass on VERSION/README/RELEASE_NOTES/HANDOFF | P3 | ◻ decommercialize-tail only (license/copyright DONE) |
...
| update-feat | **θ.4 — wire the existing θ.3 appcast data plane (`scripts/core/updates.py`) into the launcher**: poll the appcast on startup → a gentle, dismissible "update available" prompt that ALSO shows the feed's release-notes ("what's new"); link to the download; never auto-install without consent. **User decision 2026-06-08: scope = adoption-signal + a one-way release-notes channel; PRIVACY-CLEAN — NO telemetry / install-IDs / phone-home.** "Helps us" = adoption visibility from the host's ordinary appcast-poll logs + GitHub release-download counts (zero data collected from users; in keeping with the no-tracking / gift-not-pitch ethos). Sequence in the DESKTOP stage (D/E) so it ships in the v0.1.0 binaries — NOT EPUB-blocking. Touches `scripts/launcher.py` / `scripts/desktop_shell.py` + an appcast served from the site/GitHub release; coordinate with the Mac lane (don't disrupt an in-flight M1 native-window dmg). | P2 | ◻ open |
