...
- **⚠ `inject` is ADDITIVE** — it injects *missing* source notes; it does **NOT** prune asides whose source note was deleted. To drop notes from the build after deleting them from source, **regen `epub_working` from base**: `git checkout <base> -- epub_working/` → `inject --all-books` → `generate_verse_popups` → `resync_marker_glyphs`. ⚠ split files are **shared** between books — per-file base-restore is unsafe; regen all.
...
- **Audit QUEUED backlog (PLAN §4.1):** P0 ★BUGCLUSTER (code-fix **SHIPPED `c41e6d2`** — recurrence prevented; data-cleanup below) · P1 security G1 `file://` SSRF + G2 preview-XSS (**CC0 license + COPYRIGHT done**) · P2 wire the 4 dead audit-checks (`audit_dead_code/caches/deps/types`) into preflight.
- **⚠ phi/jam DATA-cleanup — do ATOMICALLY (one op) to keep source↔build mint:**
  1. **Strip** the 165 spurious `lang-hebrew` notes from `content/notes/{phi,jam}.py` — AST-span removal gives **pure deletions** (verified clean: 1815 lines, 0 insertions).
  2. **Generate Greek:** `run_greek_at_scale.py --books phi,jam --min-confidence 0.65` → **270 candidates** (phi 156, jam 114; the original χ.1 threshold was 0.65, not the 0.7 default). ⚠ **`promote.py --promote-top` misbehaved** — it marked candidates "promoted" but the notes didn't land, and a Hebrew-re-add quirk surfaced; **debug the promote/insert path before bulk-promoting** (the candidate files in `content/candidates/` are git-tracked and were stale from the buggy run — clear/regenerate them).
  3. **Regen `epub_working` from base** (inject is additive — see §3) so the orphaned Hebrew asides actually leave the build, then re-cert (§5).
  - The structural code-fix is already shipped, so the bug **cannot recur**; this is purely the one-time data correction.
