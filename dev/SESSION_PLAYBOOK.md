...
Then `SESSION_STATE.md` + `CHANGELOG.md` are updated **together** and `IN_FLIGHT.md` reflects reality. **Save cadence (RULES §4):** **local-commit freely during work** (`save.ps1` / `git commit` as slices land); **full sync only at a major milestone** or when the user says **"save / commit / push / backup / sync"** (Windows: `pwsh -File save-all.ps1 -Message "…"`; Mac: `bash dev/save_mac.sh -m "…"` — see §6.6). **"continue"/"proceed"/"go ahead" ≠ "save now"** — they advance work, not force a push; a milestone close still triggers autonomous full sync. **And never tell the user a session is "done / safe to stop / safe to /clear" — or that work is "committed" or "backed up" — without first running `git log -1` + `git status` and reporting the TRUE state (§6.7).** "Done + clean" = gates-green + tree-consistent; a local commit is a disk snapshot but **not** a full save until every sync leg landed (push + bundles on Windows). Uncommitted verified work has no snapshot — say so plainly, never reassuringly.

---

## 1. SESSION START — orient, in this order

1. **Read the bootstrap triad** (RULES §0): `dev/CLAUDE_PROJECT_RULES.md` → `dev/SESSION_STATE.md` → `dev/PLAN_2026-05-29-roadmap.md`. Then `dev/IN_FLIGHT.md` — check the `<!-- TRACKER-STATE: idle|active -->` marker. SessionStart hooks: `dev/cc-hooks/bootstrap-triad.{ps1,sh}` (installed per-box to repo-parent `.claude/hooks/` — see RULES §0).
...
6. **Save (RULES §4 two-cadence model):**
   - **During work:** local-commit slices as they land — Windows: `& ".\save.ps1" -Message "<concise; no > < → glyphs>"` (PowerShell only; `git add -A` + commit; pre-commit hook runs ruff+lint+mypy; does **NOT** push). Mac: `git add -A` + `git commit -m "…"`. Commit freely; do not wait for the user to say "save" before snapshotting verified work.
   - **At a milestone or on user "save / commit / push / backup / sync":** Windows: `& ".\save-all.ps1" -Message "…"` (`-Label <slug>` names the bundle; `-Yes` for >200-file sweeps; `-DryRun` previews). Runs all five legs in order: (1) `save.ps1` commit + truth-record rotation, (2) `git push origin` (GitLab), (3) `git push github`, (4) bundle → **E:**, (5) copy bundle → **F:**. Mac: `bash dev/save_mac.sh -m "…"` (commit if dirty + `lane_ping --before-push` + push both remotes). **`save-all.ps1` / `save_mac.sh` auto-rebase if BEHIND.** E:/F: legs optional while drives are Mac-side (STANDING). `save.ps1` alone is **not** a complete save — never report "backed up" after leg 1 only.
   - **"continue"/"proceed" ≠ "save now"** — but a milestone close still triggers autonomous full sync without asking.
7. **⚠ SAVE TRUTH GATE — before ANY "done / safe to stop / safe to /clear / saved / backed up" statement (RULES §12 audit point 5).** Run `git log -1 --oneline` + `git status --short` (+ `git status -b` after a milestone push). Report the ACTUAL state. A **local commit** = work snapshotted on disk but not yet on remotes/backups. A **full save** = every sync leg landed (both remotes ahead/behind = 0; Windows bundle exists on E:/F: when mounted). If verified work is uncommitted: **WARNING** — "⚠ NOT committed." If committed but unpushed: **WARNING** — "⚠ local only — run save-all / save_mac." **Do NOT defer local commits** — overnight work with no commits is the under-commit failure mode.
