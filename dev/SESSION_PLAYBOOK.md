...
1. **Read the bootstrap triad** (RULES §0): `dev/CLAUDE_PROJECT_RULES.md` → `dev/SESSION_STATE.md` → `dev/PLAN_2026-05-29-roadmap.md`. Then `dev/IN_FLIGHT.md` — check the `<!-- TRACKER-STATE: idle|active -->` marker. SessionStart hooks: `dev/cc-hooks/bootstrap-triad.{ps1,sh}` (installed per-box to repo-parent `.claude/hooks/` — see RULES §0). **The hook auto-starts BOTH session radars** (idempotent): **lane_watch** (60s cross-lane) + **agent_idle_radar** (120s anti-idle). Manual restart: WIN `dev/start_session_radars.ps1` · Mac `dev/start_session_radars_mac.sh`. **Never wait for user input** — pick next work from `dev/AGENT_WORK_BACKLOG.md` or `scripts/agent_idle_radar.py --next`. **Strategic replan ping (STANDING):** when radar shows P03 replan or `--replan` fires (15+ commits · 24h · PLAN changed), run `dev/STRATEGIC_REPLAN_CHECKLIST.md`, reorder backlog/queues, `--replan-done`, then immediately resume top execution item.
