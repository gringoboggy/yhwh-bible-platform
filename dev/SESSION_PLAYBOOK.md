...
- **Slow tests** — the genuinely-slow lane is real edition builds (`test_byte_stability_gate.py`); `test_matrix_psi35.py` + `test_web_filesplit.py` are the slowest non-build files. All are `slow`-tagged → `pytest -m "not slow"` skips them; targeted node-ids / `-k` are faster for single-test iteration, and under RAM pressure run **one file at a time**. (Re-measure before quoting any seconds — they rot; memory `feedback_slow_test_files`. History of the D.hang/D.slow `_stub_exports_epubcheck` fixes → `dev/archive/RULES_HISTORY.md`.)
