...
1. **Read the bootstrap triad** (RULES §0): `dev/CLAUDE_PROJECT_RULES.md` → `dev/SESSION_STATE.md` → `dev/PLAN_2026-05-29-roadmap.md`. Then `dev/IN_FLIGHT.md` — check the `<!-- TRACKER-STATE: idle|active -->` marker. SessionStart hooks: `dev/cc-hooks/bootstrap-triad.{ps1,sh}` (installed per-box to repo-parent `.claude/hooks/` — see RULES §0). **The hook auto-starts BOTH session radars** (idempotent, **always with --auto-pull** on Mac): **lane_watch** (15s cross-lane + enforces the STANDING auto-pull-on-BEHIND rule: pulls immediately on clean + `git status` behind / tracking ref lag) + **agent_idle_radar** (120s anti-idle). Manual restart: WIN `dev/start_session_radars.ps1` · Mac `dev/start_session_radars_mac.sh`. After start, the first thing the radar does on a behind + clean tree is the pull — the user never says "pull".
