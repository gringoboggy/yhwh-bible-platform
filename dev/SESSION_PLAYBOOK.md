...
1. **Read the bootstrap triad** (RULES §0): `dev/CLAUDE_PROJECT_RULES.md` → `dev/SESSION_STATE.md` → `dev/PLAN_2026-05-29-roadmap.md`. Then `dev/IN_FLIGHT.md` — check the `<!-- TRACKER-STATE: idle|active -->` marker. SessionStart hooks: `dev/cc-hooks/bootstrap-triad.{ps1,sh}` (installed per-box to repo-parent `.claude/hooks/` — see RULES §0). **The hook auto-starts BOTH session radars** (idempotent, **always with --auto-pull** on Mac): **lane_watch** (60s cross-lane + enforces the STANDING auto-pull-on-BEHIND rule: pulls immediately on clean + `git status` behind / tracking ref lag) + **agent_idle_radar** (120s anti-idle). Manual restart: WIN `dev/start_session_radars.ps1` · Mac `dev/start_session_radars_mac.sh`. After start, the first thing the radar does on a behind + clean tree is the pull — the user never says "pull".

   **"YOU ALREADY HAVE ALL THE ANSWERS" (dig first).** The project is deliberately self-contained so agents can work autonomously for long periods. When tempted to ask a question, **re-read the triad + relevant STANDING rules + run `--next` + search the folders** before considering any user interaction. Items that are not explicitly "blocked by human need" or marked deferred already have documented next steps in the backlogs, plans, and rules.

   **After any significant behavioral or rules change, step back and update the important documents** (AGENTS.md, CLAUDE_PROJECT_RULES.md, LANE_HANDOFF STANDING, SESSION_PLAYBOOK, cc-hooks, relevant specs) as part of the work. This does not wait for confirmation when the need is obvious.
