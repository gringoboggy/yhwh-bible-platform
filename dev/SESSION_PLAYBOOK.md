...
- **Tests one file at a time** (RAM pressure). **Still prefer targeted node-ids / `-k`** over the whole `tests/test_scripts.py`. The live-socket **deadlock** (D.hang) is fixed 2026-05-24 (9 tests → `ThreadingHTTPServer`; `test_ops` mocks `api_preflight`), so the suite no longer HANGS indefinitely — but a full run is still **impractically SLOW (~1 hr, killed at 48 min)**: fixing the deadlock let the suite reach tests that call the real `api_preflight()` (→ epubcheck/Java over every EPUB in `exports/`, ~1.5 GB each) + real `build_one`. **D.slow (open follow-up):** mock the real epubcheck/build in those tests (the `TestPreflightEpubcheck` aggregator tests + `build_one` smokes — same pattern as `test_ops`/`test_build_all`) to make a full run fast. ⚠ Same for `test_web_filesplit.py` + `test_matrix_psi35.py` (~23 min — live socket/build smokes). Bottom line: full-file runs COMPLETE but are slow; use node-ids for iteration; reserve full runs for patient/CI passes until D.slow lands.
