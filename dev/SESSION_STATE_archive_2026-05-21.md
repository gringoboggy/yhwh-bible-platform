...
> **➤➤➤ 2026-05-21 BUILD PIPELINE LIVE + INJECT-TAIL CLOSED to 99.21% — read FIRST; supersedes every banner below.** The lost base scripture HTML was recovered + committed (5ee2ad1) and the build makes valid EPUBs again. This session shipped three inject fixes in `scripts/inject.py` — (1) verse-end anchor fallback for KJV-vs-WEB wording divergence, (2) notes-section locator broadened to the `<p class="ch-heading">` chapter-id form + scoped by `bxx`, (3) split-boundary per-chapter section creation (`ensure_notes_section_a`, tagged `id="notes-{bxx}-c{ch}"` to avoid the cross-file dup-id that fails verify) — taking note placement **44,020 → 52,553 / 52,973 = 99.21%**. Verified: 16 tests in `tests/test_build_smoke.py`, ruff clean, `ebible verify` errors=0 / 15766 paired, valid EPUB. The residual **420 (0.79%) is base-HTML structural/versification, NOT addable content** — full remediation is PLANNED in `docs/superpowers/plans/2026-05-21-inject-tail-completion.md` (robust Strategy-B cross-file verse index → +241; render Additions-to-Esther → +73; versification audit → 69). The manuscript marathon REMAINS PAUSED (1ki5 R2 held). Live state-of-record + next-action: `dev/IN_FLIGHT.md`. For "where does X live / how does data flow" questions: `dev/MATRIX_MAP.md`. **NOTE — this file is bloated to ~874KB vs the ~900-line bootstrap-triad design target; a trim is a tracked hygiene item.**
>
> [Everything below predates the 2026-05-21 build-pipeline recovery + inject-tail work; retained as history.]

> **➤➤➤ 2026-05-20 PARALLEL-TRACK SWEEP — read FIRST; supersedes every banner below.** Today's morning audit-U-belt + multi-track scaffolding (5 ships, commits 092afd3 / 4646560 / b5a0f1c / a782f56 / 22086c8) enabled the afternoon/evening parallel-track sweep that **shipped 9 phase tags across 5 new commits** (f09081b / 2733483 / 7f2a2ac / bafb466 / febe024). Plus a final controller-side hygiene commit (THIS COMMIT) updating SESSION_STATE + IN_FLIGHT + CHANGELOG.
>
> **9 phase tags shipped today (afternoon parallel-track sweep):**
> 1. **τ.6.x.NT.scope (Track E)** — NT Ge'ez source-scope spec + multi-track runbook hygiene (f09081b)
> 2. **τ.6.x.NT.a (Track D)** — NT pre-pass + Philemon/Jude smoke × 2 langs (2733483)
> 3. **τ.6.x.2.o-t (Track C OT catchup chain)** — 7 deutero books: sir/4ba/bar/wis/paz/bel/jub Ge'ez (7f2a2ac)
> 4. **τ.F.gen1.a (Track F NEW)** — Genesis 1 EN back-translation × 2 sides + new ai-back-translation-tier4 provenance tier (bafb466)
> 5. **τ.6.x.2.u (Track C OT catchup CLOSE)** — 1 Enoch Ge'ez (final book) + lint_rules expected_geez baseline upgraded 18→26 (febe024)
> 6. **τ.6.x.5.a (Track B)** — Patrologia Orientalis pipeline (extract_patrologia_pdf.py + 65 tests) + Job 1070/1070 EXACT KJV canonical match at patrologia-printed-tier1 (uncommitted pending Ezra+Neh race resolution)
> 7. **τ.6.x.5.b (Track B)** — Esther Patrologia 167/167 EXACT KJV canonical match at patrologia-printed-tier1 (uncommitted, same race)
> 8. **τ.6.x.NT.b (Track D)** — Matthew NT-overflow re-attempt: HONEST BLOCKER REPORT (no ship; 67% overflow reduction confirmed but residual 35 > 21v tolerance; 4 leak categories diagnosed for τ.6.x.NT.c)
> 9. **OT catchup arc CLOSED** — all 7 deuterocanon books shipped this session
>
> **Render coverage delta today:** geez-tewahedo went from 16 → **27/82 rendered** (+11: sir/4ba/bar/wis/paz/bel/jub/job/phm/jud/1en; plus est_patrologia which doesn't multiply the canonical-book count by design). amharic-tewahedo went from 24 → **26/82** (+phm + jud). **2 NEW `-en` translation slots opened** (geez-tewahedo-en, amharic-tewahedo-en — Genesis 1 only as smoke).
>
> **New infrastructure surfaced today (in addition to audit-U-belt):** 1 new provenance tier (`ai-back-translation-tier4`); 1 new pipeline module (`scripts/extract_patrologia_pdf.py`, 900+ lines / 65 tests); 1 new pre-pass family (`_nt_prepass` 4-step structure-aware filter + 4 NT floor dicts in extract_parallel_pdf.py); 2 new translation-slot conventions (`*-tewahedo-en/`). The self-upgrading-matrix rule (CLAUDE_PROJECT_RULES.md §1.5) was exercised intensively — every dispatched agent updated its own tooling at ship-close before the next iteration.
>
> **Still in flight at hygiene-commit time — 6 BG agents:**
> - Track A Kings 1ki5: C-2 done + R1 NEEDS FIX + R1-fix-round shipped + C-3 R2 review running
> - Track A Samuel 1sa2: C-2 done + R1 BLOCKER (v29 fabrication detected) + fresh C-2 re-pass running
> - Track B τ.6.x.5.c Ezra+Neh Patrologia: running (banner-regex fix for apostrophe + per-book page-range OCR-probe)
> - τ.6.x.NT.c Matthew NT pre-pass refinement + re-attempt: running (4 leak fixes in scope)
> - τ.G.constitution.a standalone-Bibles editions.yaml constitution: running (per project rules §1 step 2 — constitute the two standalone Bibles)
> - τ.F.gen.b Genesis 2-5 EN back-translation continuation: running
>
> **Session metrics (incl. morning audit-U-belt):** 14 ships total; 9 commits today; lint 14·0·0 throughout; OT catchup arc CLOSED; Patrologia pipeline OPERATIONAL with 2 books shipped at exact canonical match; NT pre-pass OPERATIONAL with 2 NT books shipped (phm+jud); back-translation infrastructure OPEN with Gen 1 smoke + new tier registered.
>
> **The bootstrap-triad reading for next session should reflect the post-sweep matrix** (geez-tewahedo +11 books / amharic-tewahedo +2 / new -en slots / new patrologia-tier1 books / NT pre-pass + 2 NT books / OT catchup CLOSED). The 6 in-flight agents commit as they land; their resolutions become commits 6–11+ from today.
>
> **[SUPERSEDED 2026-05-20 EOD by the parallel-track sweep banner above — the audit-U-belt 5-ship record below is retained as morning context for the parallel-track-sweep enablement.]**
> **➤➤➤ 2026-05-20 CURRENT — read FIRST; supersedes every banner below.** τ.6.x.4.c Kings marathon: **1 Kings 1, 2, 3, 4 ✅ CALIBRATED** (47 total / 4 calibrated / 43 pending; next pending = `1ki:5`). **8 AUDIT-DRIVEN MATRIX UPGRADES SHIPPED THIS SESSION (U1+U2+U4 first batch · U5+U9+U10+U11+Stage-2.1 follow-on):** (U1) canonical `write_witness` helper validates before writing + plan FORBIDS raw `json.dump`; (U2) 1Ki4 witnesses migrated to canonical schema (geez byte-identical) + one-shot `run_1ki4_collation.py` deleted + apparatus rebuilt to 159 entries via at-scale driver; (U4) chapter classifier `chapter_profile(book, chapter)` NARRATIVE/LIST/REGNAL_FRAME with class-specific failure screens + plan METHOD NOTE 3; (U5) topology files `content/manuscript/_reviewer_context/{GG,CAM}_topology.md` persist scribal-hand references for every C-3/C-6 reviewer (~1 round saved per witness per chapter ≈ 180 rounds across remaining 94 chapters); (U9) AMBIGUOUS-PARCHMENT four-path convergence rule (METHOD NOTE 4) — no flag persists past C-7; (U10) witness-validation lint rule (12th lint check; belt-and-suspenders beyond write_witness); (U11) `dev/MARATHON_LEDGER.md` telemetry with per-chapter `{class, days, rounds, r1_defects, commit}` rows + rolling ETA modeling; (Stage-2.1) verse-count floors `FIRST/SECOND_{SAMUEL,KINGS}_VERSE_COUNTS` (3040 verses / 102 chapters canonical KJV ceiling) + drift-detection pin against `load_kjv_skeleton`. Chapter classifier extended for Samuel coverage (1Sa 13-15 REGNAL, 2Sa 8/20/23 LIST) + coverage-completeness tests (every Kings + Samuel chapter classifies; no gaps). **+31 new tests this session** (4 TestWriteWitness + 8 TestClassify/TestChapterProfile + 6 TestSamuelCoverage/TestCoverageCompleteness + 13 TestVerseCount*); manuscript-keyword regression 481/0; lint 12·0·0; ruff clean. **⏭ NEXT BATCH = 1 Kings 5** (Solomon's temple preparations / Hiram of Tyre) — **NARRATIVE-class** per `chapter_profile("1ki", 5)`, expected 2–4 rounds; GG continues from `f030v`; CAM continues into `f128r` (fresh CUDL pull). Awaiting user go-ahead per the established single-chapter cadence (memory `feedback_marathon_pacing`). **FOURTH SHIP 2026-05-20 — WHOLE-PROJECT audit (cross-track scaffolding for all 5 parallel tracks):** `scripts/render_coverage.py` whole-project inventory tool (geez 16/82, amharic 24/82, manuscript 4 in-flight, patrologia 5 books in 4 PDFs ready, +5 tests); `scripts/core/canonical_verse_counts.py` KJV-skeleton-derived per-book counts for 81 KJV-anchored books + 6 Tewahedo distinctives (+10 tests; NT 27/27 resolve); `scripts/core/provenance_tiers.py` formal tier registry (manuscript-collation-tier2 / patrologia-printed-tier1 / ocr-tier3 / kjv-skeleton / digitized-critical-edition — last one was an existing in-repo undeclared tier surfaced by the new lint check); 2 new lint checks (`render_coverage_no_regression` + `provenance_tier_known` — 14·0·0 total); `docs/superpowers/specs/2026-05-20-patrologia-ingest-design.md` Track-B design spec (PO 2/9/13/23 PDFs → 1ch/2ch/ezr/neh/job render; reuses pymupdf+Tesseract). **5 PARALLEL TRACKS IDENTIFIED + SCAFFOLDED:** A. Manuscript (in flight) · B. Patrologia (PDFs ready, spec shipped) · C. Parallel-PDF Ge'ez catchup (paused at Sirach) · D. Amharic NT cadence (paused) · E. NT Ge'ez (source TBD). Each track has canonical verse-count floor + provenance tier + render-coverage tracking + lint regression detection. +15 new tests this ship; session total +67 tests across 4 ships.

**THIRD SHIP 2026-05-20** (user said "all"): U6 `escalate_if_unbounded` round-bound helper (+11 tests, plan C-3/C-6 calls it between rounds — executor-level enforcement of METHOD NOTE 3); U7 `scripts/acquire_cudl_master.py` CUDL pre-pull tool (region-tile past 1503×2000, stitch via Pillow, smoke-tested live against view 215 = 7760×10328; bulk 3 GB pull left to user as one-line for-loop command); pre-screened C-2/C-5 `screen_witness_for_class_failures` (+10 tests; flags `❈` body, `ይ`/`ደ` family curated list, `እስ`/`እለ` for LIST+REGNAL); U8 CAM-heavier prompt asymmetry codified in C-5/C-6; U12 hybrid cadence METHOD NOTE 5 (NARRATIVE continuous, LIST+REGNAL stop-and-check-in); 11 review files moved into `dev/marathon_reviews/1ki4/`; Samuel marathon scaffolding KICKOFF-READY (55 total / 4 calibrated / 51 pending; all audit-U-belt upgrades apply). **+52 new tests this session** (12+19+21); manuscript regression 546/0; lint 12·0·0. **Audit-U-belt arc COMPLETE this session — all 10 originally-proposed levers shipped.** ⏭ Next pending chapter = `1ki:5`; awaiting user go-ahead. Samuel marathon can run in parallel if user authorizes.

> **➤➤➤ 2026-05-19 CURRENT (HISTORICAL) — superseded by 2026-05-20 above; retained for handoff context.** τ.6.x.4.c Kings marathon: **1 Kings 1, 2, 3 ✅ CALIBRATED & committed** (1ki1 `9bb9976`; 1ki2 `40fd8a8` — base SURFACE-TO-USER event resolved to base=CAM per ratified decision-of-record as segmentation-granularity artifact; 1ki3 GG `2036fb2` + CAM `9fa3ffe` + final calibration THIS COMMIT — full C-1…C-9 done 2026-05-19: GG = single folio f030r 27v converged after 5 fresh reviewer rounds + glyph adjudicator + 4 fix rounds; CAM = f127r-M→f127r-R→f127v-L L1-6 27v converged after 1 review + source-of-truth adjudicator on 6 diagnostic loci + 1 page-verified fix + R2 spot-check clean; C-7 metrics MATCH ratified pattern — semantic 96.43% (27/28; v28 = recensional minus marked-gap NOT fabricated), ww both-confident 43.77% (well below ≈unity-CONTRADICT bar), base=CAM, lacuna {gg:1,cam:0,both:0} honest, token-conservation 0/28 fail; C-8 byte-identical re-derivation; VERDICT MATCHES). **⏭ NEXT BATCH = 1 Kings 4 C-1 — DO NOT start without explicit user go-ahead** (user cadence revised mid-session 2026-05-19 to single-chapter batches: "stop and commit when 1 Kings 3 is done"; memory `feedback_marathon_pacing` reinforced). The 1ki3 convergence proved CAM heavier than GG in this scribal family (denser hand, more red-rubric register, more line-end glyph-drop ambiguity); expect comparable rigor on 1ki4. CAM 1ki3 ended at f127v-L L6/7 with `ሰሎምን` immediately before the `ነግሥ ላዕለ ኩሉ እስራኤል ✣ ክፍል ፰` next-section rubric → 1ki4 CAM likely begins f127v-L L7 and extends into f128r (NOT yet on disk; fresh CUDL pull expected). A base SURFACE-TO-USER (engine §3.3 clause-3) or any CONTRADICTS trigger still stops immediately regardless. Manifest = durable cross-session ledger: **47 total / 3 calibrated / 44 pending** (next pending = `1ki:4`). Historical banners below retained for arc history; the live resume point + cadence are HERE.**
>
...
> **STAGE 1 / task #5 = 1 Kings 1 (the safety-stop) — ✅ COMPLETE;
> VERDICT = MATCHES (2026-05-18; marathon continues continuously, NO
> user check-in — model user-ratified 2026-05-17; C-10 was 1ki1-only,
> no further safety-stop):** both witnesses GG **53 v** (`eaf2063`) +
> CAM **52 v** (`75ba7b2`) blind-transcribed (isolated opus) + 3-round
> adversarially converged; the safety-stop codified **3 systemic
> failure classes** into the plan's per-chapter METHOD NOTE before the
> bulk: (1) harmonization-to-printed-text (CARDINAL RULE + termination
> bar); (2) **column/folio-boundary dropped-scripture mis-labelled
> "recensional minus"** — worst class; caught on CAM, restored, 50→52 v
> (boundary make-or-break now mandatory); (3) contamination-screen bug +
> `save.cmd` `git add -A` scratch footgun (`580d7fe` hygiene +
> `.gitignore` backstop + OS-temp mandate). **C-7 collate → C-8
> adversarial review (PASS) → C-9 manifest+commit → C-10 verdict ALL
> DONE:** `content/manuscript/kings/collation/1ki1_collation.json` —
> **semantic 100.0% (53/53), W↔W strict 14.86% / skeleton 19.78% /
> both-confident 29.89%, uncertainty 0.26%, lacuna {gg:0,cam:1,both:0},
> base=CAM** (honest decision-of-record `_pick_base`, no fitted
> constant). C-8 independently re-implemented every metric from
> `verses[].alignment[]` → **byte-identical**; token-conservation 0/53
> fail; v30 CAM-illegible = 0 GG substitution (apparatus only); v53
> extent-minus = marked-gap (GG not merged); validator `OK` both. One
> **cosmetic** `lacuna_counts_note` prose imprecision (≤0.02pp, honesty-
> conservative, can't inflate) → honest tracked WARN per
> `no-reassert-ratified-bar`; shipped Samuel-byte-identical engine
> **not** mutated mid-marathon (separate cleanup w/ Samuel-equivalence
> regression). Manifest 1ki:1 → `calibrated`; collation+manifest
> committed `9bb9976` (witnesses already immutable). Bi-directional
> rule: ALL MATCHES criteria hold (semantic 100≥95 · both-confident
> 29.89≪90 on clean folios · base=CAM · no contradiction — not ≈unity,
> base did not flip to GG, semantic not <95); MORE divergence than
