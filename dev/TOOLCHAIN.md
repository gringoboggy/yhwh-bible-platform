...
| **Kindle Previewer 3** (Mac, PINNED **3.104.0**) | the LOCAL Kindle-conversion oracle (same pipeline class as Send-to-Kindle/KDP; names the E-code in ~minutes vs ~50-min STK uploads) — the K-KIN bisect runner | ✅ installed 2026-06-11 under the standing install grant: official pkg `d2bzeorukaqrvt.cloudfront.net/KindlePreviewerInstaller.pkg` (sha256 `ecc3866a…`, Developer ID **AMZN Mobile LLC**, notarized, verified) → `pkgutil --expand-full` payload → `~/Applications/Kindle Previewer 3.app` (user-local, no sudo — the Temurin/kepubify layout) | KDP "Kindle Previewer" page → pkg → payload extract | `"$HOME/Applications/Kindle Previewer 3.app/Contents/MacOS/Kindle Previewer 3" -help` |
