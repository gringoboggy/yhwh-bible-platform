...
> - **✅ WAVE 6 DONE (`71bdd6e2` golden + `build_play.py`):** golden re-baselined (diff = 9 hashes only; determinism re-verified 9/9; epubcheck 0/0/0/0; spot-confirmed badge-at-text-end 0/548 + Arabic embed + clean separators) → pushed for Mac cross-OS. **4 device artifacts rebuilt + re-staged** (re-QA guide `QA/READ-ME-device-reQA-round2.md`): Apple `YHWH-v1.0.0-apple.epub` (epubcheck 0/0/0/0) · Kindle `YHWH-v1.0.0-kindle.epub` (verify_kindle_m4b clean + epubcheck 0/0/0/0) · Play `YHWH-v1.0.0-play.epub` (A2 endnotes, verify_play_safe clean + epubcheck 0/0/0/0) · Kobo `G:\YHWH-koboQA.kepub.epub` (verify_kr2 ALL GREEN; epubcheck confirming). Count UNCHANGED (90,181). **▶ PENDING (user): re-QA the 4 readers → cut v1.0.0.** Do NOT cut the tag.
>
