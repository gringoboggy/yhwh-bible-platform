...
> - **1 Kings 3 IN PROGRESS.** C-1 locate ✅ (entirely on GG folio **f030r**: col L line 2 → M → R ~line 19; bounds proven — START after the 1ki2 tail + red `✣`, incipit `ወተሐመዐ ሰሎሞን ሰፈርዖን ንጉሥ ግብጽ`; END `…ከመ ይግበር ኩነኔ ርትዕ ።።` before the red `✣ ክፍል` divider + the 1ki4 officials list `ወስሎሞን ንጉሥ ነግሠ … አዘርያስ ወልደ ሳዶቅ`). C-2 blind GG transcribe ✅ **controller-verified** — `content/manuscript/kings/calibration/1ki3_witnessGG.json`, 27 v contiguous, 455 tok, `validate_witness` OK, contamination screen CLEAN, 1 honest `⟦illegible⟧` (col-L line-32 blot) + 1 `damaged` (v15), bijection OK. **WIP-committed at this checkpoint — NOT yet C-3-reviewed, MUTABLE until C-3 approves; manifest `1ki:3` stays `pending`.**
> - **⏭ RESUME = 1 Kings 3 C-3** (adversarial GG review + fix-loop, fresh isolated subagents, both METHOD NOTES) → C-4 acquire CAM hi-res (per the 1ki2 C-4 finding the 1ki3 CAM text begins at the top of **f127r-M**, incipit `ወፈኀራ ስሎሞን … ፈርዖን ንጉሥ ግብጽ`; the `MS-ADD-01570_f127r_*_hires.jpg` ~80MP master is already on disk from the 1ki1 pull — likely reuse f127r, maybe also f127v) → C-5 blind CAM → C-6 adversarial CAM + fix-loop → C-7 collate → C-8 collation review → C-9 manifest flip + commit. **NO C-10** (1ki1 only). Watch for the base SURFACE-TO-USER again if CAM segments coarsely (resolve identically: segmentation artifact ⇒ base=CAM decision-of-record, but it STILL stops for the user).
