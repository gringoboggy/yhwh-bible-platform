...
<!-- TRACKER-STATE: v1.0.0 DEVICE-QA REMEDIATION active (2026-06-29). From the user's 4-reader QA round; tracker dev/audit/device-qa-v1.0.0-2026-06-28.md. WIN landed ALL pre-rebuild code: 9345c842 removed 1,364 machine-authored User notes (corpus 91,712->90,348); 3c46a46d Apple LTR spine + single merged badge off-Kobo + clean separators; b3588727 badge-nowrap + Edition-ID->copyright; e4210aff Kindle teleport fix (M4b relocates study vnotes-* + translation vnote-* popups to reachable backmatter endnotes, matrix routes via make_kindle_m4b, m4b default-on, 17/17 green); batch-4 Kindle body justify. The 2 CRITICALs (Apple reads-backwards, Kindle teleport) fixed in source. NEXT (WIN) = the ONE full rebuild of every edition x format (flagship eink last/solo) -> re-baseline 9-KJV golden (note removal + separator/badge/Edition-ID change the cells) -> capture exact new note count (~90,191) -> push golden + announce count. MAC lane handed (LANE_HANDOFF): (1) NOW v1.0.0 .dmg notarize; (2) after golden push, cross-OS 9/9 verify; (3) after count, public count cascade + website deploy. Lane-sync monitor armed. Do NOT cut the tag. Round-16 + the pre-tag Windows lane = DONE (history below). -->
...
> **▶▶ 2026-06-29 (LATEST) — v1.0.0 DEVICE-QA REMEDIATION (active arc). Tracker `dev/audit/device-qa-v1.0.0-2026-06-28.md`. Tag NOT cut.**
...
> exact new note count (≈90,191) → push the golden + announce the count. **▶ MAC (lane in `dev/LANE_HANDOFF.md`):** (1) NOW —
