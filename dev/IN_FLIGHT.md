...
**PROGRESS — Stage 1 / task #5 = 1 Kings 1 (safety-stop) — BOTH WITNESSES ✅ DONE:**
...
**RESUME POINTER:** 1Ki1 next = **C-7 (controller) collate** — run the shipped Phase-2 tool on the two approved witnesses: `mc.load_kjv_skeleton('1ki',1)` → `mc.collate(gg,cam,kjv,book='1ki',chapter=1)` → `mr.reconcile(collation)`; write `content/manuscript/kings/collation/1ki1_collation.json`; capture metrics (semantic-pass %, both-confident W↔W %, base witness). GG=53 v vs CAM=52 v with CAM-side recensional minuses already looks like the **expected distinct-recension pattern** (consistent with the ratified Samuel finding) — but the verdict is C-10's, post-collation. Then **C-8** adversarial collation review (recompute every metric from `alignment[]`; lacuna reconcile == evidence `⟦illegible⟧`; fix-loop) → **C-9** flip manifest 1ki:1→`calibrated` + final commit (witnesses+collation+manifest together) → **C-10 the bi-directional safety-stop verdict**: semantic ≥95% + base=CAM + distinct-recension (both-confident materially <90%) + no contradiction → MATCHES → continue the queue (Task #14 validator-hardening before the 1ki2-22 bulk; #14 blocks #6); CONTRADICTS (≈unity, or base flips to GG, or semantic<95%) → STOP & surface to user. Manifest: 1ki:1 still `pending` (correct — flips to `calibrated` only at C-9); other 46 chapters untouched.
