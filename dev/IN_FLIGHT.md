...
> **▶▶ 2026-06-29 (LATEST) — DEVICE-QA ROUND-2: Waves 4 + 5 DONE/committed/pushed; Wave 6 (rebuild + golden re-baseline) IN PROGRESS. Tag NOT cut.**
...
> - **▶ WAVE 6 (active):** golden `--regen` running (9 KJV cells) → re-verify (check-mode determinism) → push for Mac cross-OS 9/9 → rebuild + re-stage 4 device artifacts (ethiopian-tewahedo M1 tablet · M4 kindle-m4b · M5 play · **M3 eink kepub LAST+SOLO OOM-aware**; delete old `G:\YHWH-koboQA.kepub.epub`; Kobo→`G:`, rest→`OneDrive\Desktop\QA`) → user re-QA → user cuts v1.0.0. Golden-affecting set complete = B·X·D·E-Apple·G·H-c·H-a; count UNCHANGED (90,181). Do NOT cut the tag.
>
