...
**RESUME POINTER:** 1Ki1 safety-stop **COMPLETE — VERDICT MATCHES**; the ratified diplomatic-parallel + base=CAM model is confirmed on Kings (on cleaner text than the original Samuel gate). Marathon **continues continuously, NO user check-in** (decision-inheritance: model user-ratified 2026-05-17; C-10 was 1ki1-only — no further safety-stop). **NEXT = Task #14 (validator-hardening): fold the corrected non-Ethiopic contamination screen (whitelists the sanctioned `⟦illegible⟧` sentinel) into `scripts/core/manuscript_records.validate_witness`** — TDD; MUST keep the 4 immutable Samuel goldens + the 71-test manuscript regression byte-identical/green. **Task #14 blocks Task #6 (the 1ki2-22 → 2ki1-25 bulk).** After #14 lands: run the bulk continuously, one chapter at a time (C-1…C-9 per chapter; no C-10 — 1ki1 only), manifest-tracked via `run_manuscript_collation_at_scale.py --track kings` (now: total 47 / collated 1 / pending 46). Then Stage 2 = Phase-3 render `geez-tewahedo/1ki.py`+`2ki.py` + apparatus + `manuscript-collation-tier2`. Local commit only — no push, no zip.
