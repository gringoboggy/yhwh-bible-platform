...
>
> **▶ REMAINING:**
> 0. **✅ WIN OPEN round-13 items DONE (2026-06-23, this session — `04340574` #9 · `c85a772b` #5 · `243efb7` #6; all green + epubcheck 0/0/0/0):** #9 inject `escape_attr` (escapes `& < > "`, preserves `'` — "Nave's …" is a real title; 4 title-attr sites) · #5 dropped 3 dead `REPO` (re-verify caught 2 spec false-positives — `api/exports`+`api/preflight` `REPO` are LIVE dev-server paths, kept) · #6 `ZipInfo.create_system=0` in all EPUB writers (catholic-study built → all 383 entries `create_system=0`). **`sources_base` lazy-PATH = conservative DEFER** (read-only published data → in-bundle read is correct for a frozen app; not a bug). **Remaining round-13 = the joint merge w/ Mac's half: char-vs-byte #2 · #7 `audit_popup_formula` · device-QA.**
