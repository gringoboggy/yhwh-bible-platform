...
**RESUME POINTER:** 1Ki1 safety-stop **COMPLETE — VERDICT MATCHES**; Task #14 validator-hardening **SHIPPED + reviewed** (bulk-gate cleared); the 3 C-9-broken Stage-0 tests **fixed state-aware + reviewed** (manuscript suite **81/0**). The ratified diplomatic-parallel + base=CAM model is confirmed on Kings; marathon **continues continuously, NO user check-in** (decision-inheritance: model user-ratified 2026-05-17; C-10 was 1ki1-only — no further safety-stop). **NEXT = plan Task #6 — the bulk: `1ki` ch 2→22 then `2ki` ch 1→25 (46 chapters), one chapter at a time, the full C-1…C-9 per-chapter procedure VERBATIM (NO C-10 — 1ki1 only), each chapter's both witnesses isolated-opus blind-transcribed + adversarially converged (~3 rounds; the TWO METHOD NOTES — anti-harmonization + column/folio-boundary make-or-break — apply to every C-2/C-3/C-5/C-6), CAM hi-res via CUDL-IIIF (`cudl-iiif-access`), collate via the shipped Phase-2 tool, manifest `{ref}`→`calibrated` + commit per chapter.** Start at **`1ki` chapter 2** (`ref=1ki2`; GG source folder `GAPS/2_Kings/GG-00106/1-Kings/` folios f028v–f040v; CAM low-res locating band `GAPS/2_Kings/Cambridge-Add-1570/Kings_f126r…f146v.jpg`). This is the explicitly **multi-session** body of the marathon — the manifest (`run_manuscript_collation_at_scale.py --track kings`: total 47 / collated 1 / pending 46) is the durable cross-session ledger; resume the queue from the first `pending` chapter. After all 47 `calibrated`: S1-QA, then Stage 2 = Phase-3 render `geez-tewahedo/1ki.py`+`2ki.py` + apparatus + `manuscript-collation-tier2`. Local commit only — no push, no zip.
