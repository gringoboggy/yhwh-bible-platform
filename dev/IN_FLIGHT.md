...
> - **1 Kings 3 IN PROGRESS — GG witness APPROVED & IMMUTABLE.** C-1 locate ✅ (entirely on GG folio **f030r**: col L line 2 → M → R ~line 19; bounds proven — START after the 1ki2 tail + red `✣`, incipit `ወተሐመዐ ሰሎሞን ሰፈርዖን ንጉሥ ግብጽ`; END `…ከመ ይግበር ኩነኔ ርትዕ ።።` before the red `✣ ክፍል` divider + the 1ki4 officials list `ወስሎሞን ንጉሥ ነግሠ … አዘርያስ ወልደ ሳዶቅ`). C-2 blind GG transcribe ✅. **C-3 adversarial GG review ✅ CONVERGED & APPROVED** (2026-05-19): **5 fresh independent reviewer rounds + 1 glyph adjudicator** (R1→13 defects → fix-1; R2→4 new → fix-2; R3→6 new incl. 1 CRITICAL dropped word `እን` → fix-3; R4→6 of which 3 manufactured ping-pong against R3 → focused glyph adjudicator with same-scribe stroke calibration settled 6 contested loci definitively → fix-4 applied 3 adjudicator-confirmed changes; **R5 APPROVED CLEAN** — both column turns (L→M, M→R) traced glyph-by-glyph + every line-end junction in all 3 columns, no dropped/added scripture, v8 col-L L32 `⟦illegible⟧` + v15 col-M L17 `damaged` flags honest, bijection intact, validator `OK`, screen `CLEAN`). Total: 10 verses corrected across 4 fix rounds (v2/v3/v4/v6/v8/v14/v15/v16/v17/v19/v20/v23/v24/v27 — primarily harmonization-to-printed class with one boundary-region dropped word, all faithful-to-parchment per CARDINAL RULE; sub-glyph orthographic variation read as-inked NOT pushed to standard). The `feedback_reverify_conservative_nogo` pattern recurred — R4 was half-right (3 of 6 readings correct, 3 manufactured), settled by source-artifact adjudication rather than blind ping-pong. **GG witness content final — `content/manuscript/kings/calibration/1ki3_witnessGG.json` IMMUTABLE; manifest `1ki:3` stays `pending` (becomes `calibrated` at C-9).**
> - **⏭ RESUME = 1 Kings 3 C-4 acquire CAM hi-res** (per the 1ki2 C-4 finding the 1ki3 CAM text begins at the top of **f127r-M**, incipit `ወፈኀራ ስሎሞን … ፈርዖን ንጉሥ ግብጽ`; the `MS-ADD-01570_f127r_*_hires.jpg` ~80MP master is already on disk from the 1ki1 pull — likely reuse f127r, maybe also f127v if 1ki3 extends past f127r) → C-5 blind CAM (fresh isolated opus, both METHOD NOTES) → C-6 adversarial CAM + fix-loop → C-7 collate via shipped Phase-2 tool → C-8 collation review → C-9 manifest flip 1ki:3→calibrated + final commit. **NO C-10** (1ki1 only). Watch for the base SURFACE-TO-USER again if CAM segments coarsely (resolve identically: segmentation artifact ⇒ base=CAM decision-of-record, but it STILL stops for the user).
> - **⚑ CADENCE (user instruction, 2026-05-19): run the marathon in SMALL BATCHES of ~2–3 chapters, then STOP and report for an explicit go-ahead before the next batch.** This SUPERSEDES the plan/older-banner "marathon continues continuously, no user check-in" wording (user instructions outrank the plan; memory `feedback_marathon_pacing`). Within a chapter the C-1…C-9 pipeline may chain; the check-in boundary is every ~2–3 chapters (the next natural stop after resume: finish 1ki3, then 1ki4, 1ki5 → STOP & report). A base SURFACE-TO-USER / any CONTRADICTS trigger still stops immediately regardless.
> - **Durable ledger:** `content/manuscript/kings/manifest.yaml` = **47 total / 2 calibrated / 45 pending** (1ki:1, 1ki:2 calibrated; next pending = `1ki:3`). `run_manuscript_collation_at_scale.py --track kings` dry report mirrors this. Local commit only (remote deleted) — no push, no zip.
