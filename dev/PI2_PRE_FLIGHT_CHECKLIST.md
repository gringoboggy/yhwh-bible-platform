...
2. Sample verses (e.g. mq1 11:1 + gen 1:1) emit popups with all
   five languages.
3. All 3 OTHER canon-differentiated tradition editions (catholic-study,
   evangelical-reformed, eastern-orthodox) preserve their existing
   popup_languages_default.
...
| τ.7.x.b Amharic Exodus ingest | τ.7.x.b | ✓ SHIPPED 2026-05-15 | Second τ.7.x.* sub-ship: amharic-tewahedo/ex.py created with 947 verses at 78.1% coverage via the τ.7.x.a pipeline (text-layer + paragraph_mode + renumber) — only deltas were EXODUS_VERSE_COUNTS floor (40 ch / 1213 v) + structural_map.exodus block (pdf_page_range [86, 160] verified by Ex 40:36-38 closing → Lev 1:1 opening content inspection). Chapters 1-32 fully populated + 33 partial (6/23) + 34-40 empty (τ.6.x.3 audit-handoff). End-of-Exodus colophon preserved. Confirms τ.7.x.a pipeline as the canonical τ.7.x.* per-book template; 6th instance of single-key back-link annotation pattern (first signaling pipeline-template-reuse). |
| τ.7.x.c Amharic Leviticus ingest | τ.7.x.c | ✓ SHIPPED 2026-05-15 | Third τ.7.x.* sub-ship: amharic-tewahedo/lev.py created with 802 verses at **93.4% coverage — HIGHEST τ.7.x.* coverage yet** (vs Gen 85.3% + Ex 78.1%; Leviticus has short verse-dense ritual-law chapters with minimal cross-ref leakage). Pipeline reused VERBATIM from τ.7.x.b — only deltas: LEVITICUS_VERSE_COUNTS floor (27 ch / 859 v) + structural_map.leviticus block (pdf_page_range [161, 213] verified by Lev 27:34 @ p212 + Num 1:1 @ p214 content-boundary inspection). Chapters 1-25 fully populated + 26 partial (23/46) + 27 empty (τ.6.x.3 audit-handoff). Third consecutive τ.7.x.* ship with zero parser API change — firmly establishing the τ.7.x.a template; 7th instance of single-key back-link annotation pattern. |
| τ.7.x.d Amharic Numbers ingest | τ.7.x.d | ✓ SHIPPED 2026-05-15 | Fourth τ.7.x.* sub-ship: amharic-tewahedo/num.py created with 1107 verses at **85.9% coverage** (sits between Gen 85.3% and Lev 93.4%, well above Ex 78.1%; Numbers narrative-dense profile of long census + itinerary chapters interleaved with shorter narrative chapters yields ~85-86% recovery band mirroring Genesis). Pipeline reused VERBATIM from τ.7.x.c (which itself reused τ.7.x.b which reused τ.7.x.a) — only deltas: NUMBERS_VERSE_COUNTS floor (36 ch / 1288 v; Masoretic + LXX + Tewahedo agreement; Vulgate 16:36-50 → 17:1-15 repartitioning NOT followed) + structural_map.numbers block (pdf_page_range [214, 287] verified via Deuteronomy title `ኦሪት ዘዳግም` scan at p291 + content-boundary inspection: Num 36 daughters-of-Zelophehad closes at p287 + Deut 1:1 opens at p288 + explicit `ኦሪት ዘዳግም` title at p289). Chapters 1-30 fully populated + 31 partial (47/54) + 32-36 empty (τ.6.x.3 audit-handoff). End-of-Numbers colophon preserved at renumbered ch 31:47. **80% of the Pentateuch closed under Amharic-first sequencing** (gen + ex + lev + num shipped). Fourth consecutive τ.7.x.* ship with zero parser API change — decisively establishing the τ.7.x.a template; 8th instance of single-key back-link annotation pattern. |
| τ.7.x.e Amharic Deuteronomy ingest — §8.1 Pentateuch arc-close | τ.7.x.e | ✓ SHIPPED 2026-05-15 | Fifth τ.7.x.* sub-ship: amharic-tewahedo/deu.py created with 781 verses at **81.4% coverage** (sits between Exodus 78.1% and Genesis 85.3%). Pipeline reused VERBATIM from τ.7.x.d — only deltas: DEUTERONOMY_VERSE_COUNTS floor (34 ch / 959 v; KJV/LXX/Vulgate-aligned enumeration) + structural_map.deuteronomy block (pdf_page_range [288, 348] verified via Joshua title `መጽሐፈ ኢያሱ` scan + content-boundary inspection: Deut 34 epilogue at p348 + Joshua 1:1 at p349). Chapters 1-27 fully populated + 28 partial (62/68) + 29-34 empty. End-of-Deuteronomy colophon preserved at renumbered ch 28:62. **CLOSES the §8.1 Pentateuch arc under Amharic-first sequencing** — gen + ex + lev + num + deut = ALL 5 BOOKS OF TORAH SHIPPED in amharic-tewahedo. NINTH §8.1 arc-close instance overall + FIRST in τ-cluster — codifies per-book-cadence + Amharic-first Pentateuch-arc-close as durable structural pattern (prior 8 §8.1 all γ-cluster). **Pentateuch combined coverage: 4945/5853 = 84.5% across all 5 books of Torah; 274 PDF pages consumed (0-348)**. Fifth consecutive τ.7.x.* ship with zero parser API change — five-ship zero-API-delta is the strongest possible refactor-stability signal short of a code-frozen contract; 9th instance of single-key back-link annotation pattern. |
| τ.7.x.f Amharic Joshua ingest — post-Pentateuch historical-books arc-open | τ.7.x.f | ✓ SHIPPED 2026-05-15 | Sixth τ.7.x.* sub-ship: amharic-tewahedo/jos.py created with 483 verses at **73.4% coverage — LOWEST τ.7.x.* coverage to date** (slightly below Exodus's 78.1%; Joshua's long tribal-allotment chapters with dense Hebrew place-name lists + publisher-added Judges-bridge narrative on page 390 consume parser bandwidth). Pipeline reused VERBATIM from τ.7.x.e — only deltas: JOSHUA_VERSE_COUNTS floor (24 ch / 658 v; KJV/Hebrew + LXX agreement) + structural_map.joshua block (pdf_page_range [349, 390] verified via Judges 1:1 opening at p391 content inspection). Chapters 1-18 fully populated + 19 partial (13/51 — includes publisher-bridge-narrative leakage + colophon) + 20-24 empty. End-of-Joshua colophon `ለእግዚአብሔር ለዘለዓለሙ የተናገረው መጽሐፍ መላ አሜን ሆሣዕ (ኢያሱ) ተፈጺመ` preserved at parser terminal output. **OPENS the post-Pentateuch historical-books arc under Amharic-first sequencing** — FIRST τ-cluster ingest after the §8.1 Pentateuch arc-close at τ.7.x.e; historical-books arc will span Joshua → Judges → Ruth → 1-4 Kingdoms → 1-2 Paralipomena → Ezra/Nehemiah → Esther under LXX/Tewahedo ordering. **Pentateuch + Joshua combined: 5428 verses / 6511 expected = 83.4% across 6 books; 316 PDF pages consumed (0-390)**. Sixth consecutive τ.7.x.* ship with zero parser API change — six-ship zero-API-delta extends template stability across Pentateuch + Joshua. NEW residual class: publisher_bridge_narrative_residual (page 390 contains Judges 3:7-12 forward-reference summary BEFORE Judges 1:1 at p391; τ.6.x.3 audit will flag bridge-narrative leakage as non-canonical). NEW NULL-FORMAL-TITLE-BANNER pattern: Joshua first τ.7.x.* book without explicit `መጽሐፈ X` book-title-banner in PDF (publisher uses `ኦሪት ዘኢያሱ` running-header form consistently); 10th instance of single-key back-link annotation pattern. |
| τ.7.x.g-z Amharic remaining books | τ.7.x.g → τ.7.x.z | ⬜ next-phase under D1-a per-book cadence | τ.7.x.g (Amharic Judges) is next-up — continues the post-Pentateuch historical-books arc. Re-uses τ.7.x.a+b+c+d+e+f pipeline; needs JUDGES_VERSE_COUNTS floor (21 ch / 618 v per KJV) + structural_map.judges block (pdf_page_range starting at 391 per τ.7.x.f boundary inspection — Judges 1:1 confirmed at p391). |
...
+ τ.6.x.1.B + τ.6.x.1.C + τ.6.x.1.D + τ.6.x.2.D + τ.7.x.a + τ.7.x.b
+ τ.7.x.c + τ.7.x.d + τ.7.x.e + τ.7.x.f (SIX τ.7.x.* sub-ships;
**§8.1 Pentateuch arc CLOSED at τ.7.x.e + post-Pentateuch
historical-books arc OPENED at τ.7.x.f** — gen + ex + lev + num +
deut + jos = 6 books shipped; 316 PDF pages consumed (0-390); 5428
verses / 6511 expected = 83.4% combined coverage) shipped;
remaining gates τ.7.x.g-z (under D1-a per-book cadence; τ.7.x.g
Judges continues the post-Pentateuch historical-books arc) +
τ.6.x.2+ (after τ.7.x completion) + τ.6.x.3 (after both arcs). All
remaining gates are now data-ingest + operator cross-check work;
no Claude-side technical or publisher-direction blockers remain.**
