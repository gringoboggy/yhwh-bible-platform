...
| τ.7.x.f-z Amharic remaining books | τ.7.x.f → τ.7.x.z | ⬜ next-phase under D1-a per-book cadence | τ.7.x.f (Amharic Joshua) is next-up — OPENS the post-Pentateuch historical-books arc under Amharic-first sequencing (τ.7.x.e §8.1 arc-close marks Pentateuch boundary; τ.7.x.f opens next canonical unit). Re-uses τ.7.x.a+b+c+d+e pipeline; needs JOSHUA_VERSE_COUNTS floor (24 ch / 658 v per KJV) + structural_map.joshua block (pdf_page_range starting at 349 per τ.7.x.e boundary inspection — Joshua 1:1 confirmed at p349). |
...
+ τ.7.x.c + τ.7.x.d + τ.7.x.e (FIVE τ.7.x.* sub-ships; **§8.1
Pentateuch arc CLOSED under Amharic-first sequencing** — gen + ex
+ lev + num + deut = all 5 books of Torah shipped) shipped;
remaining gates τ.7.x.f-z (under D1-a per-book cadence; τ.7.x.f
Joshua opens the post-Pentateuch historical-books arc) + τ.6.x.2+
(after τ.7.x completion) + τ.6.x.3 (after both arcs). All remaining
gates are now data-ingest + operator cross-check work; no Claude-
side technical or publisher-direction blockers remain.**
