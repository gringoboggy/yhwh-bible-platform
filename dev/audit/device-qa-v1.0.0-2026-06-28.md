...
- ✅ **Kindle teleport (CRITICAL)** — `scripts/core/kindle_post.py` M4b extended to relocate BOTH note families to backmatter glossaries with cross-file hrefs: study `vnotes-*` → "Study Notes" glossary (badges retargeted) + translation `vnote-*` → "Original-Language Witnesses" glossary (`vn-link` verse-number anchors retargeted), one shared splitter skeleton. Matrix Kindle cell now routes through `make_kindle_m4b`/`verify_kindle_m4b` (`build_format_matrix._apply_kindle_post`); `build_kindle.py` m4b **default-on** (`--no-m4b` = bare strip). No content dropped. `tests/test_kindle_m4b.py` 17/17 green; all 4 files compile + ruff-clean. (batch 3, commit `e4210aff`). Verifies on the rebuilt Kindle artifact (step 4).
- ✅ **Kindle body justification** — `body { text-align: justify }` added to `_KINDLE_M4B_CSS` (`kindle_post.py`). The base stylesheet justifies `p.verse-p` (Apple/Kobo honour it) but Amazon's KFX falls back to ragged-left without a body-level signal; the rule is Kindle-only and additive (the reader's own alignment toggle still wins). (batch 4)
- ✅ **Kindle ToC pills + title-page split** — already handled in the M4b path: `_flatten_toc_pills` rewrites the `<ol class=toc-chapters>` pills to a horizontal `<p class=toc-chapter-row>` (KFX drops `<li>` inline-block → vertical stack) with `.toc-chapter-row a { margin:0 .35em }` spacing; `.book-title-page { page-break-after:auto }` already cut the title page 3→2. Final spacing/1-page tuning needs the device → **device re-QA on the rebuilt Kindle artifact** (don't tune KFX pagination CSS blind).
...
- ⏳ **Post-rebuild only (need the rebuilt artifact):** residual mid-verse line breaks (WS1 — enumerate via `dev/audit_verse_formatting.py` on the rebuilt output, then extend the WS1 re-join); Kindle pill-spacing / title-page-to-1 device tuning; Play ToC expandable (attempt; likely a Play reader limitation). **★ All pre-rebuild WIN code is now in — the next step is the ONE full rebuild (step 4).**
