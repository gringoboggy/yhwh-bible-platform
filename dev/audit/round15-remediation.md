...
| **D3** | HIGH | MAC | Douay/Clementine-Vulgate **Ps 2:13 + Ps 4:10 silently DROPPED** — `vulgate_to_kjv`→`None`, real scripture missing from the `vulgate` parallel popup | ✅ **FIXED** (Mac, `29321fad`) — `_VULGATE_PSALM_FIXES` `(2,13)→(2,12)` + `(4,10)→(4,8)` (same trailing-fold shape as `(135,27)→(136,26)`); regen CONFINED to douay/vulgate `psa.py` (2:12/4:8 gain their tails; appendix + `_meta` preserved). `dev/audit_versification_coverage.py` exhaustive gate GREEN (selftest non-taut; breadth pass found 0 more drops). Pinned: `test_psalm_fix_maps[(2,13)/(4,10)]` + `test_round15_source_gates`. ⚠ **STORE-FIX ONLY — the popup is BAKED** (WIN finding `c7e2d8e4`): `_BAKED_NOW` + no edition sets `translation_id` ⇒ the build never re-reads the store, so the shipped Latin popup STILL drops Ps 2:13/4:10 AND the store edit does NOT change the golden (no re-stamp from it). **WIN to complete:** surgical `epub_working/` base edit of the Ps 2:12/4:8 `vnote-vulgate`/`vnote-douay` `<p>` (Mac-supplied exact tails; **Q1 resolved — KEEP the `]`**) → `G1 --regen` → Mac cross-OS-verify. See `round15-d3-baked-popup-finding.md`. **✅ WIN RE-BAKE DONE:** surgical 4-append base edit (Ps 2:12/4:8 `vnote-vulgate`+`vnote-douay` in `index_split_032.html`; confined 4-line diff; `check_nested_anchors` 0 + `test_nested_anchors` 10/10; the `]` repairs a dangling unclosed `[`). Restored Latin tail SHIPS in the built epub (`beati omnes qui confidunt in eo` ×1). **G1 golden RE-STAMPED** (6 cells = everywhere+tablet × 3 editions; the 3 kindle cells unchanged — kindle compacts the parallel-Latin popup). ▶ Mac: cross-OS verify the re-stamp. |
| **D1** | HIGH | MAC | release asset-set integrity (orphan/missing/dup SHA256SUMS, retired-SKU EPUBs) | ✅ **FULLY DONE.** Gate (Mac, `29321fad`) `dev/audit_release_assets.py` had found **100 retired-SKU EPUBs attached to live v0.1.0** (5 retired editions × 20). **★ LIVE RE-CUT DONE (WIN, 2026-06-27, user-authorized "do it"):** deleted all 100 retired-SKU EPUB assets from the GitHub v0.1.0 release (GitHub = canonical home; GitLab v0.1.0 has no file assets) + rebuilt `SHA256SUMS.txt` (186→86 lines). **D1 gate now PASS:** `attached=87 sums=86 grammar=80 legacy=2 platform=4 meta=1 retired=0 orphan_sums=0 missing_sums=0`. No collateral — the public website (`website/`) references **0** retired-SKU assets (only the 80 active grammar EPUBs, all preserved). v1.0.0 will be clean by construction (build only emits the 4 active editions). |
| **D2** | MED | WIN | rendered xref/noteref anchor whose target id is absent from the reader's spine piece; `check_xrefs.py` scans only pre-split superset | ✅ **DONE (WIN, 2026-06-27).** `check_xrefs.py:52-53` regex hardened (`2afa6126`, zero-diff) **+ G3 xref-class breakout**: scripture `v-`/`ch-` cross-references are now counted + failure-bucketed separately (`xref_links`/`xref_fails`) from the noteref popup bulk, with a dedicated **`--min-xrefs` floor** (the ~45k noteref bulk keeps `--min-links` satisfied even if every scripture xref were dropped). **Wired `--min-xrefs 10000` into the per-build gate** (`test_round14_build_gates.py`). Verified on FRESH builds: ethiopian eink **88,541 xrefs** · catholic-study eink **55,774 xrefs**, **0 dead** (G3 PASS); all 4 per-build gates (G3/G4/G5/D8) PASS on the fresh catholic-study build; +2 unit tests (`test_xref_class_broken_out`, `test_xref_floor_catches_loss_hidden_by_noteref_bulk`). |
| **D5** | HIGH | WIN | `_stream_glossary_pieces_from_bytes` byte-streamer reached in PROD only by the >64 MB ethiopian flagship glossary, never byte-verified at scale | ✅ **DONE (WIN, 2026-06-27)** — **CHECK A**: real **255 MB** flagship `index_split_900` monolith str==from-file **byte-identical (690/690 pieces, 255,162,686 B each)**; byte branch confirmed firing (255 MB > 64 MB; the doc "~480 MB" was a stale str-side estimate, doc-conflict resolved). **CHECK B**: G5 on the fresh flagship eink build **PASS** (`max_inner_cp=399171 < 400000` cap, 0 over-cap, 83 book-heads, 0 multi-bookhead, 30148 atoms == 30148 distinct). Permanent real-threshold regression `test_file_split::TestStreamGlossaryFromFile::test_real_threshold_byte_branch_identical_to_str_at_scale` (slow, 82 MB synthetic, **no monkeypatch**). **+ found+fixed a confound:** G5 opt-in `--reference-split` false-FAILed on REAL (post-`rewrite_links`) builds (it re-splits post-rewrite content the build split pre-rewrite with reserved headroom) → now detect-post-rewrite + skip + WARN (`_REWRITTEN_HREF_RE`) + regression `test_reference_split_skips_post_rewrite_built_pieces`. Detail: `round15-d5-glossary-byteproof.md`. |
...
| **D8** | MED | WIN | emitted nav/opf order never asserted vs config; `enrich_nav_chapters` sorts chapters ascending → a swap is invisible to ToC + auditor | ✅ **DONE** (WIN, `e57e6c64`) — `dev/audit_canonical_order.py`: reads `ch-bNN-cMM` in DOCUMENT order → chapters ascend per book (catches the swap the sorted nav masks) + books in bp order (appendix is NOT spine-demoted; "nav 83 vs spine 86" is NAV-only) + ncx gapless. Validated on the built catholic-study epub (307 pieces/1299 ch/75 books PASS — no order defect) + 5 synthetic RED/drift tests; wired into the per-build gate. |
| **D9** | MED | WIN | fresh ethiopian `.kepub` inline `verse-notes` ids missing `-sN` tail; `_POPUP_ASIDE_RE` can pass vacuously if kepubify reorders attrs | ✅ **DONE** (WIN, `93d2fb85`) — `dev/audit_kepub_revid_family.py`: inline (`verse-notes`, not `study-glossary-cat`) ids MUST be `-sN`, navigate (`study-glossary-cat`) bare OK, + liveness self-check (FAILs vacuously if class-first popup regex matches 0 while popup asides exist). Census on the flagship kepub: **0 inline-bare · rev=606 navigate · liveness 36329** ⇒ guard #19 is **by-design** (the "792 bare rev" scare = the navigate family), no live regression. 4 synthetic tests. (Fresh-kepub final run deferred; conclusion is layout-determined — eink backmatter has 0 inline popups.) |
...
| **MAC** | D3 ✅ · D1 ✅ (gate Mac + live re-cut WIN) · D6 ✅ · D7 ✅ + cross-OS verify every WIN build | `scripts/core/versification.py`, `content/translations/{douay-rheims,vulgate-clementine}/psa.py`, `dev/audit_versification_coverage.py`, `dev/audit_release_assets.py`, `dev/audit_canon_bookcount.py`, `dev/audit_migration_idempotence.py` |
...
- **2026-06-27 D5 DONE (WIN)** — flagship glossary byte-streamer proven: CHECK A real 255 MB monolith str==from-file byte-identical
  (690/690 pieces); CHECK B G5 PASS on the fresh flagship eink build (399171 < 400000 cap). Permanent real-threshold regression
  added (no monkeypatch). Found+fixed G5 `--reference-split` false-FAIL on real post-rewrite builds (detect+skip+WARN). Report
  `round15-d5-glossary-byteproof.md`. **▶ Mac: cross-OS verify the D5 build (rebuild flagship eink on macOS → G5
  PASS + str==from-file via the new slow test).**
- **2026-06-27 D2 DONE (WIN)** — G3 xref-class breakout: scripture `v-`/`ch-` cross-references counted + failure-bucketed
  separately (`xref_links`/`xref_fails`) from the noteref bulk + a dedicated `--min-xrefs` floor; **wired `--min-xrefs 10000`
  into the per-build gate**. Verified on FRESH builds (ethiopian 88,541 xrefs / catholic-study 55,774 xrefs, 0 dead; all 4
  per-build gates PASS) + 2 unit tests. **▶▶ ROUND-15 ✅ ALL DIMENSIONS COMPLETE (D1–D9 both lanes).**
- **2026-06-27 ALL cross-OS verify COMPLETE (Mac, `4f69aa05`+`405eda85`)** — D3 golden 9/9 Win↔Mac, D5 new tests + 82MB
  byte-branch, D8/D9, and D2 (G3 xref-breakout `xref=45057`/`xref_fails=0` + per-build gate) all PASS on macOS.
- **2026-06-27 D1 LIVE RE-CUT DONE (WIN, user-authorized "do it")** — removed all 100 retired-SKU EPUBs + their SHA256SUMS
  lines from the live GitHub v0.1.0 release; D1 gate now PASS (87 assets, 0 retired, bijection clean); no website breakage.
  **★★ ROUND-15 IS NOW FULLY CLOSED — no remaining follow-ups.** (The v1.0.0 release gate / next audit round = separate, future.)
