# IN_FLIGHT archive — 2026-05-20 23:30 EST

Content trimmed from dev/IN_FLIGHT.md during the 3rd-crash cleanup. Each section was superseded by newer state at trim time; preserved here for git-blame context and rare historical retrieval. **Always read IN_FLIGHT.md first** — this file is NOT live.

---

## Superseded recovery banners (2026-05-20: 22:30 2ND-CRASH → 22:00 RECOVERY → 16:45 EOD-hygiene)

>
> **[SUPERSEDED 2026-05-20 23:30 EST by 3RD-CRASH banner above — 22:30 2ND-CRASH banner retained for context; its commit 7a20eca documenting the (now-disproved) flat 3-cap remains in history but its rule is overridden]:**
...
> - **Track A Kings 1ki5** (this marathon's next chapter): C-2 GG-blind-transcribed (13v) → C-3 R1 NEEDS FIX (9 hard defects: 5 CRITICAL + 4 MAJOR; v6 `ጠቢላሰ`→`ጠቢበ ፡ ላዕለ` line-break trace + dropped token; v7-v8 `✣` body-cross mis-parsed as fidel `እ`; v10 `ስኪራም`→`ለኪራም` topology §2 ለ/ስ family; v12 `ወረነወ`→`ወፈነወ` NEW ፈ/ረ family) → R1 fix-round APPLIED → **C-3 R2 review running**. Boundary verified clean (start ክፍል ፡ ፳ ወ፬ ❈ rubric on f030v; end before 1Ki6:1 `ወእምዝ ፡ አመ ፡ ኮነ ፡ ፬ ፻` on f031r). 13-verse count acceptable per ክፍል sub-numbering pattern (no `ምዕራፍ` on these folios, only ክፍል).
> - **Track A Samuel 1sa2** (parallel marathon track): C-2 GG-blind-transcribed (32v) → C-3 R1 **BLOCKER** (v29 content appears FABRICATED — parchment at f004r-M L14+ actually reads 1Sa3:11-12; 12+ hard defects + ~30 word-level errors + ❈/✣ over-normalization + damaged-bottom undersampling) → **fresh C-2 re-pass running** (NOT a fix-round; reviewer recommended substantial re-transcription with corrected methodology: ≥6x per-verse zoom, preserve post-rubric ❈, multi-illegible honesty, NEW ይ/ግ/ን topology family applied, v29 specific watchout).
> - **Track B τ.6.x.5.c Ezra+Neh Patrologia**: running — extracting both books from PO 13 fasc 5 Pereira 1919 via the newly-shipped extract_patrologia_pdf.py; also fixing the BANNER_RE apostrophe-form bug surfaced by τ.6.x.5.b.
> - **τ.6.x.NT.c Matthew NT pre-pass refinement + re-attempt**: running — applying the 4 leak-category fixes diagnosed by τ.6.x.NT.b (broaden pericope-strip char-class for OCR-mangled section numerals; add unbracketed-cross-ref pattern; add Ethiopic-letter-ratio garbage filter; OCR-stub micro-verse handling). Will re-attempt Matthew + optionally ship 1-2 more singleton NT books (Mark/Luke) if Step 4 succeeds.
> - **τ.G.constitution.a standalone-Bibles constitution**: running — per project rules §1 sequence step 2 (constitute the two standalone editions). Adds `standalone-geez` + `standalone-amharic` entries to content/editions.yaml at the full Tewahedo canon, with `popup_translation` = the new `*-tewahedo-en` slots. Audit confirms the other 9 editions don't have ge'ez/amharic popups (per rules §1).
> - **τ.F.gen.b Genesis 2-5 EN back-translation continuation**: running — extends the τ.F.gen1.a smoke (Gen 1 × 2 sides) to chapters 2/3/4/5. Same source-faithful (NOT KJV-aligned) discipline; bleedthrough markers; EOTC marginalia preserved.
>
> **OT catchup queue COMPLETE** (all 7 deuterocanon Ge'ez books shipped at τ.6.x.2.o-u: sir/4ba/bar/wis/paz/bel/jub/1en).
>
> **Render coverage 2026-05-20 EOD:** geez-tewahedo 27/82 (+11 today); amharic-tewahedo 26/82 (+2 today); 2 new -en slots opened (Gen 1 only).
>
> **Lint 14·0·0** throughout the parallel-track sweep. Each commit's pre-commit hooks (ruff format --check + scripts/lint_rules) gated cleanly. Today's `git add -A` footgun was avoided via targeted `git add <files>` per ship for atomic per-track commits.
>
> **Resume pointer (next session):** read SESSION_STATE.md (just updated, new top banner) → IN_FLIGHT.md (this file) → CHANGELOG.md (sixth ship section added). The 6 in-flight agents' completion notifications will dispatch follow-on commits (#6 Ezra+Neh, #7 Kings 1ki5 close after R2/C-4/C-5/etc., #8 Samuel 1sa2 close, #9 Matthew NT.c, #10 standalone-Bibles, #11 Gen 2-5 BT).
>
> **[HISTORICAL — morning audit-U-belt context, preserved for parallel-track-sweep enablement]:** FIVE audit-driven ship batches landed inside this marathon on 2026-05-20 — audit-U-belt arc COMPLETE + WHOLE-PROJECT scaffolding shipped + final-audit fixes (METHOD NOTES reordered 3/4/5, GAPS/ moved INSIDE repo + gitignored for backup self-containment, self-upgrade rule codified in CLAUDE_PROJECT_RULES.md, MULTI_TRACK_RUNBOOK_2026-05-20.md shipped). 5 parallel tracks IDENTIFIED + SCAFFOLDED + RUNBOOK-READY. See `dev/MULTI_TRACK_RUNBOOK_2026-05-20.md` for per-track kickoff commands. First batch: U1 canonical writer + U2 1Ki4 schema migration + U4 chapter classifier. Second batch: U5 topology files + U9 AMBIGUOUS rule + U10 witness-validation lint + U11 MARATHON_LEDGER + Stage-2.1 verse-count floors + Samuel classifier coverage. Third batch: U6 escalate_if_unbounded round-bound helper + U7 acquire_cudl_master pre-pull tool + pre-screened C-2/C-5 self-check helper + U8 CAM-heavier prompt asymmetry + U12 hybrid cadence METHOD NOTE 5 + reviews reorganized into `dev/marathon_reviews/1ki4/` + Samuel marathon scaffolding KICKOFF-READY. **+52 new tests this session** (12+19+21); manuscript regression 546/0; lint 12·0·0. Marathon continues; next pending chapter = `1ki:5`.
>
> **Samuel marathon (τ.6.x.4.b follow-on) — KICKOFF-READY.** Same scribal hands (GG-00106 + Cambridge Add. 1570), same engine + collation tool, same shipped Phase-2 infrastructure as Kings. Manifest at `content/manuscript/samuel/manifest.yaml` shows **55 total / 4 calibrated / 51 pending** (1sa1, 1sa3, 1sa17, 2sa11 = calibration gate chapters; 51 remain for the marathon). All audit-U-belt upgrades apply automatically to Samuel: chapter classifier covers all 55 chapters (1Sa 13-15 REGNAL, 2Sa 8/20/23 LIST, rest NARRATIVE); topology files are shared across tracks; write_witness, screen_witness_for_class_failures, escalate_if_unbounded all work with `track="samuel"`; at-scale driver via `--track samuel`. **To start Samuel marathon: pick a starting chapter (e.g., `1sa2`, the first uncalibrated chapter), call `chapter_profile("1sa", 2)` for class, execute the per-chapter procedure template.** Samuel is independent of Kings — can run in parallel if user authorizes; otherwise resume after Kings is complete (1Ki5-22 + 2Ki1-25).


---

## Kings marathon — prior detail (superseded by CURRENT RESUME POINT at top of trimmed IN_FLIGHT.md)


> **⏭⏭⏭ PRIOR RESUME POINT (2026-05-19) — superseded by 2026-05-20 above; retained for handoff context.**
> - **1 Kings 1 ✅ calibrated** (commit `9bb9976`). **1 Kings 2 ✅ calibrated** (commit `40fd8a8`): full C-1…C-9 done; the engine `_pick_base` clause-1 raised a base SURFACE-TO-USER (CAM 27 seg vs GG 46) → conservative re-verify showed a **segmentation-granularity artifact** (token-extent CAM/GG = 1.033, semantic 46/46, lacuna 0/0/0, CAM C-6-certified complete, C-8 token-conservation exact) → **user decided 2026-05-19: base=CAM per the ratified 2026-05-17 decision-of-record**; recorded honestly in `dev/CALIBRATION_2026-05-19-kings-1ki2.md` (engine output left byte-identical per `no-reassert-ratified-bar`). Witnesses immutable: GG `c3db778`, CAM `b0d939e`.
> - **1 Kings 3 ✅ CALIBRATED.** Full C-1…C-9 done 2026-05-19: GG witness on single folio **f030r** (3 cols L→M→R; 27 v); CAM witness on **f127r-M → f127r-R → f127v-L L1–L6** (27 v; no fresh CUDL pull — f127r/v hi-res masters already on disk from 1ki1 C-4; staged byte-identical under `_1ki3_hires.jpg` naming). **GG C-3 took 5 fresh adversarial reviewer rounds + 1 glyph adjudicator + 4 fix rounds** (R1→13 defects → fix-1; R2→4 new → fix-2; R3→6 new incl. 1 CRITICAL dropped word `እን` → fix-3; R4→6 of which 3 manufactured ping-pong against R3 → focused glyph adjudicator with same-scribe stroke calibration settled 6 contested loci definitively → fix-4 applied 3 adjudicator-confirmed changes; **R5 APPROVED CLEAN**). 10 GG verses corrected (v2/v3/v4/v6/v8/v14/v15/v16/v17/v19/v20/v23/v24/v27 — primarily harmonization-to-printed class with one boundary-region dropped word). **CAM C-6 took 1 first-pass review + 1 source-of-truth glyph adjudicator + 1 page-verified fix + 1 fresh R2 spot-check clean.** Pattern verdict MIXED: C-5 had real word-level systematic misreads at Loci B/C/D (`ቅድሜክ`-6-word-compression / `ፀርከ`+`አላ ሰአልክ` split / `ሠናይ` 1st-order); C-6 over-extended into standard-form harmonization at Loci E/F (page actually inks `አይከፍልኒ` — distinct from both reviewers). 21 CAM verses corrected; tokens 489→461; uncertain 44→8. Both witnesses IMMUTABLE: GG `2036fb2`, CAM `9fa3ffe`. **C-7 collation MATCHES the ratified pattern:** semantic 96.43% (27/28; v28 = recognized recensional minus, both witnesses 27v vs KJV skeleton 28v, marked-gap NOT fabricated per honesty contract); ww strict 20.22% / skeleton 28.78% / **both-confident 43.77%** (well below the ≈unity-would-CONTRADICT 90% threshold — distinct-recension signal, healthy); uncertainty 1.74% (8/461 base=CAM); lacuna `{gg:1, cam:0, both:0}` matches witness ⟦illegible⟧ evidence exactly; **base = CAM per ratified 2026-05-17 GO + spec-revision §3.3 clause 2** (extents not materially different GG 27v vs CAM 27v; NO base SURFACE-TO-USER). **C-8 independently re-implemented every metric from `alignment[]` → byte-identical; token-conservation 0/28 fail; v28 marked-gap (`gap=True, geez=[⟦illegible⟧]`) NOT fabricated; apparatus 27 entries well-formed; VERDICT = MATCHES.** Memories reinforced: `feedback_reverify_conservative_nogo` (R4 half-right pattern recurred on GG side; CAM C-6 same pattern), `no-reassert-ratified-bar` (ratified bar applied as MATCHES not per-build fail), CARDINAL RULE cuts both ways (rejected both pseudo-archaism and standard-form-harmonization in CAM convergence).
> - **⏭ NEXT BATCH = 1 Kings 4 C-1** (awaiting explicit user go-ahead per the single-chapter cadence). Per the original plan template: GG source folder `GAPS/2_Kings/GG-00106/1-Kings/` (1ki4 begins after 1ki3 END on f030r-R ~line 21, then the red `✣ ክፍል ፴` divider, then the officials list `ወስሎሞን ንጉሥ ነግሠ … አዘርያስ ወልደ ሳዶቅ`; trace into f030v as needed). CAM begins at the next position after 1ki3 ends on f127v-L L6/7 (CAM inks `ነግሥ ላዕለ ኩሉ እስራኤል ✣ ክፍል ፰` immediately after the 1ki3 tail — likely f127v continues into f128r; the next folio is NOT yet on disk from prior pulls — fresh CUDL pull expected for at least f128r). Full C-1…C-9 per the per-chapter procedure; NO C-10. Both METHOD NOTES every C-2/C-3/C-5/C-6. The 1ki3 marathon proved CAM is more labor-intensive than GG (CAM is denser, has more red-rubric register, more line-end glyph-drop ambiguity); expect comparable rigor on 1ki4.
> - **⚑ CADENCE (user instructions, 2026-05-19):** original "~2-3 chapter batch" + mid-batch revision to **single-chapter "stop & commit when 1 Kings 3 is done"** (memory `feedback_marathon_pacing` reinforced — user wants the marathon run slowly with check-ins). Batch is now CLOSED at 1ki3. **Do NOT start 1ki4 without explicit user go-ahead.** A base SURFACE-TO-USER / any CONTRADICTS trigger still stops immediately regardless.
> - **Durable ledger:** `content/manuscript/kings/manifest.yaml` = **47 total / 3 calibrated / 44 pending** (1ki:1, 1ki:2, 1ki:3 calibrated; next pending = `1ki:4`). `run_manuscript_collation_at_scale.py --track kings` dry report mirrors this. Local commit only (remote deleted) — no push, no zip.
...
**PROGRESS — Stage 1 / task #5 = 1 Kings 1 (safety-stop) — ✅ COMPLETE; VERDICT = MATCHES (marathon continues continuously, no user check-in):**
- **C-1 locate ✓ / C-2 blind GG transcribe ✓ / C-3 adversarial GG review ✓ APPROVED** — `content/manuscript/kings/calibration/1ki1_witnessGG.json`, **53 v**, GG f028v(3 cols)→f029r. Converged in **3 fix-rounds** (harmonization-to-printed-text class). Validator `OK`, screen `CLEAN`, bounds+honesty faithful-to-parchment. **Immutable + checkpoint-committed** (`eaf2063`).
- **C-4 acquire CAM hi-res ✓** — CUDL-IIIF, 4 folios f126r/v + f127r/v at full ~80MP (7760×10328), pixel-exact stitch, folio identity double-confirmed (manifest label + f106r=view215 cross-check, zero drift). `GAPS/2_Kings/Cambridge-Add-1570-hires/MS-ADD-01570_f{126r,126v,127r,127v}_1ki1_hires.jpg`. CC BY-NC, Cambridge UL.
- **C-5 blind CAM transcribe ✓ / C-6 adversarial CAM review ✓ APPROVED** — `content/manuscript/kings/calibration/1ki1_witnessCAM_hires.json`, **52 v**, CAM f126r(3 cols)→f126v-L; 1 honest `⟦illegible⟧` (v30, abraded red rubric). Converged in **3 fix-rounds**; round-1 caught **2 CRITICAL accidental column/folio-boundary scripture omissions** mis-labelled "recensional minuses" (f126r col1→col2 lost std v15/16; col3→f126v-L lost std v42) → restored from the parchment + re-segmented 50→52 v; round-2 verified both restorations complete+faithful; round-3 final 2-item delta clean. Validator `OK`, screen `CLEAN`, contiguous, bounds correct (no 2Sa24 leak, 1Ki2 excluded). CAM witness immutable + checkpoint-committed (this turn).
- **Method bug FIXED:** the non-Ethiopic screen was false-flagging the mandated `⟦illegible⟧` sentinel (GG slipped by with 0 illegible; CAM has 1) — corrected screen now whitelists the sentinel; codified in the plan METHOD NOTE; Task #14 will fold it into `validate_witness`.
- **TWO METHOD NOTES codified into the plan's per-chapter template** (applies to every remaining C-2/C-3/C-5/C-6): (1) anti-harmonization CARDINAL RULE + termination bar + corrected screen + 2–3-round convergence; (2) **column/folio-boundary OMISSION** make-or-break — C-5 must trace text-continuity across every column/folio turn (verse boundary never mid-word); C-6 must independently verify every claimed recensional minus is GENUINE at the boundary, not a dropped-scripture artefact. The bulk marathon now runs with both guards. This is exactly what the safety-stop existed to learn.
- **C-7 collate ✓ / C-8 adversarial collation review ✓ PASS / C-9 manifest+commit ✓ / C-10 verdict ✓ MATCHES** — `content/manuscript/kings/collation/1ki1_collation.json` (53-verse canonical spine). **semantic 100.0% (53/53), W↔W strict 14.86% / skeleton 19.78% / both-confident 29.89%, uncertainty 0.26%, lacuna {gg:0,cam:1,both:0}, base=CAM** (honest decision-of-record `_pick_base`, no fitted constant). C-8 independently re-implemented every metric from `verses[].alignment[]` → **byte-identical**; token-conservation 0 failures/53 rows; lacuna bijection holds; v30 CAM-illegible = 0 GG substitution (apparatus only); v53 CAM extent-minus = `marked-gap`, GG not merged; validator `OK` both witnesses. **One cosmetic prose imprecision** in the engine-emitted `lacuna_counts_note` (≤0.02pp, honesty-conservative, can never inflate) — recorded as honest tracked WARN per memory `no-reassert-ratified-bar`; shipped Samuel-byte-identical engine **not** mutated mid-marathon (separate cleanup, run w/ Samuel-equivalence regression). Manifest 1ki:1 → `calibrated`; collation+manifest committed `9bb9976` (witnesses already immutable `eaf2063`/`75ba7b2`). Verdict logic + evidence in `dev/CALIBRATION_2026-05-17-kings-1ki1.md`. The bi-directional rule: ALL MATCHES criteria hold (semantic 100≥95 · both-confident 29.89≪90 on clean text · base=CAM · no contradiction trigger — not ≈unity, base did not flip to GG, semantic not <95); MORE divergence than ratified Samuel-1 on CLEANER folios = *stronger* confirmation of the same ratified distinct-recension model, not a new contradiction (conservative re-verify per `feedback_reverify_conservative_nogo` survived).
- **Task #14 (validator-hardening) ✅ SHIPPED + two-stage reviewed** — folded the corrected non-Ethiopic contamination screen into `scripts/core/manuscript_records.validate_witness` (`_screen_non_ethiopic`; ALLOWED = Ethiopic U+1200–U+137F + ASCII space + rubric-cross `✣` U+2723 + whole-token `⟦illegible⟧` sentinel; catches Latin/mojibake e.g. `f` U+0066). **Empirical correctness check:** `✣` U+2723 occurs **94×** in the immutable Samuel goldens' `geez` (`_geez_to_tokens`-sanctioned divider) — the METHOD-NOTE one-liner's U+1200–137F-only set was provably too strict; ALLOWED includes `✣` so all **9 immutable Samuel calibration goldens + both 1ki1 witnesses still validate `ok=True`** (verified independently). TDD: 10 new tests `TestValidatorNonEthiopicScreen` in `tests/test_manuscript_collation.py` (incl. a non-vacuity 94-cross guard). Pure additions (existing 7 validator checks byte-identical; `(not e), e` contract intact). Commits `f25fc98` + `79ba184` (code-review nits: dropped rotting task-ref, post-gate direct-indexing). Spec review **SPEC-COMPLIANT** + quality review **APPROVED**. **Task #14 GATE CLEARED → the 1ki2-22/2ki1-25 bulk (plan Task #6) is now UNBLOCKED.**
- **Stale Stage-0 tests FIXED state-aware (per rules §8) ✅ + two-stage reviewed** — C-9's legitimate `1ki:1`→`calibrated` flip broke 3 Stage-0 `tests/test_manuscript_kings.py` tests that default-assumed "all 47 pending forever" (the exact ψ.3-precedent anti-pattern). Redesigned to per-status-branch + accounting invariants (non-vacuous; rename `_47_pending`→`_47_chapters` / `_reports_47_pending`→`_accounting`); Samuel back-compat pins byte-unchanged. Commits `ca82fd0` + `1d2c517` (code-review: dropped a rotting `§8` tag from a test comment + added positive-pin durability rationale). Spec **SPEC-COMPLIANT** + quality **APPROVED**. **Manuscript regression now `-k "manuscript|…"` = 81 passed / 0 failed** (was 78/3); lint 0-fail; ruff clean. Suite is green for the bulk.

**RESUME POINTER:** 1Ki1 safety-stop **COMPLETE — VERDICT MATCHES**; Task #14 + Stage-0-test fixes **SHIPPED + reviewed** (manuscript suite **81/0**, bulk-gate cleared); **plan Task #6 (the bulk) is STARTED.** Marathon **continues continuously, NO user check-in** (decision-inheritance: model user-ratified 2026-05-17; C-10 was 1ki1-only — no further safety-stop). The bulk = `1ki` ch 2→22 then `2ki` ch 1→25 (46 ch), one chapter at a time, the full **C-1…C-9 per-chapter procedure VERBATIM (NO C-10 — 1ki1 only)**, each witness isolated-opus blind-transcribed + adversarially converged (~2–3 rounds; the TWO METHOD NOTES — anti-harmonization CARDINAL RULE + column/folio-boundary make-or-break — apply to every C-2/C-3/C-5/C-6), CAM hi-res via CUDL-IIIF (`cudl-iiif-access`), collate via the shipped Phase-2 tool, manifest `{ref}`→`calibrated` + commit per chapter.

**✅ `1ki` chapter 2 (`ref=1ki2`) GG witness — C-2+C-3 DONE; GG witness CONVERGED & APPROVED at C-3 Round 4 + IMMUTABLE (content final `c3db778`); ▶ now at C-4.** **C-1 locate ✓** (controller: 1Ki2 starts f029r MIDDLE col after the 1Ki1 tail; spans f029r-mid → f029r-R → f029v(L/M/R) → f030r-L; ends before the 1Ki3 narrative on f030r-L; folios clean/undamaged; GG source = f029r,f029v,f030r). **C-2 blind GG transcribe ✓** (isolated opus → `content/manuscript/kings/calibration/1ki2_witnessGG.json`, **46 v**, validator `OK`; 1Ki1 tail + 1Ki3 + the **red `እትርጓቚ` interpretive-gloss layer** all correctly excluded). **C-3 adversarial GG review — ROUND 1 done + fix applied (NOT yet approved):** reviewer CONFIRMED both boundaries + the gloss-exclusion is a *genuine* separate red editorial register (the make-or-break structural calls are sound), but found a **CRITICAL v28 dropped-black-scripture-at-the-gloss-boundary** (METHOD NOTE 2 class — caught) + systematic glyph misreads (`ለ`→`ስ`, `ብእሴ`/`ብእሲ`, v2 dropped word, v46). Fix-round-1 (fresh isolated agent, on-page readings, CARDINAL RULE) **restored v28 black scripture, ran the systematic `ለ`/`ስ` pass with cross-glyph verification, corrected 13 verses (2,4,5,6,8,23,27,28,32,33,35,44,46), validator `OK`, kept parchment `እሜሳዶ` over the expected `አሜሳዶ` (CARDINAL RULE upheld, honest-flagged)**. **WIP checkpoint-committed `d4a5368`** (explicitly NOT approved/immutable; manifest `1ki:2` still `pending`; the witness is MUTABLE until C-3 approves). **C-3 ROUND 2 done + fix applied (still NOT approved):** round-2 reviewer independently **re-verified the v28 restoration is faithful & complete** (the worst defect class is CLOSED) and confirmed v5 `እሜሳዶ`=parchment `እ` (CARDINAL RULE correctly upheld), v4 `ፍኖትየ`, v45 honest — but caught a **PARALLEL v8 red-gloss leak** (gloss tokens `ወይእዜኒ`/`ኤጣዕዱ` wrongly pulled INTO v8 — same class as v28, reverse direction) + bounded glyph defects (`ለ`/`ስ` residual in untouched v7; v35 `ኃይሉ`→`ሣራዊት`; v45 `ጽልው`→`ድልው`; v46 `ስሰሉሞን`→`ስለሉሞን`; 3 flag-pointer misalignments; v3/v44/v15 sub-glyph). Fix-round-2 (fresh isolated agent, on-page readings ≥30×, CARDINAL RULE) **removed the v8 gloss-leak, ran a final all-46-verse `ለ`/`ስ` sweep (found only 2 more: v34 `ወስመንበሩ`→`ወለመንበሩ`, v4 `ብእሴ`→`ብእሲ`), corrected 10 verses (3,4,7,8,15,34,35,44,45,46), fixed all flag pointers, made disciplined verified NON-changes (did not over-correct genuine `ስ`), validator `OK`, v28 byte-identical**. **WIP checkpoint-committed `e24f2c9`** (still NOT approved/immutable; manifest `1ki:2` still `pending`). Convergence trajectory strong: round-1 (1 critical+systematic) → round-2 (1 critical+bounded) → only 2 extra in the full sweep; 1 honest sub-glyph residual flagged (v35 `ሣራዊት` wa-form `ዊ`/`ዌ`). **C-3 ROUND 3 done + fix applied:** fresh adversarial reviewer of HEAD `e24f2c9` found **9 defects** — 2 CRITICAL (**v14** dropped scripture: word `ገጸሙ` absent + `ወአንበሩ፡እግዘም` misread of `ወአንቤየ፡ግብእ`; **v42** fabricated clause `እስአሚ✣ወይቤሎ፡ስበቱ፡ስንሐልኩከ` + `ለሰሜ`→`እስአሚ`) + 7 MAJOR (a systematic `ሳዕስ`→`ሳዕለ` harmonization the round-2 `ለ`/`ስ` sweep had **introduced/left** in v27/v32×2/v33×2/v37/v44×2, plus residual `ለ`→`ስ` misreads v34 `ሰሳም`→`ሰላም`, v39 `ህስው`→`ህለዉ`); v8 leak-removal + v28 + all 10 round-2 fixes independently re-confirmed faithful. Fix-round-3 (fresh isolated agent, reviewer's exact on-page readings, same-scale `ለ`/`ስ` calibration, CARDINAL RULE, tight scope) **applied 8 defects exactly + adjudicated DEFECT 5's final word: v33 `ለዓዓም` confirmed rounded `ለ` on the ink — REFUTED the reviewer's tentative `ሳዓዓም`** (CARDINAL RULE + reviewer's own fallback), recomputed tokens (v14 +1, v42 −4), validator `OK`, scope-diff = exactly verses {14,27,32,33,34,37,39,42,44}, all others byte-identical. WIP committed `c3db778`. **C-3 ROUND 4 = fresh independent adversarial reviewer → ✅ APPROVED:** both CRITICALs confirmed faithful by glyph-by-glyph boundary trace; the 7 `ስ`/`ለ` fixes correctly directioned with **no over-correction** (v5/v23 independently re-verified still genuine rounded `ሳዕለ`); v33 final word definitively `ለዓዓም` on the ink; scope integrity confirmed (v8/v9, v28, round-2 v3/v4/v7/v15/v45/v46, v35 untouched & faithful; chapter boundaries + red `እትርጓቚ` gloss-exclusion intact); validator `OK`, screen `CLEAN` (the `✣` U+2723 divider bare-allowlist flag is the known non-defect artifact — shipped `validate_witness` whitelists it per Task #14), 46 contiguous verses, bijection intact. One honest note (NOT a defect, TERMINATION-BAR-accepted): v35 line-final "upon" fidel count is resolution-limited on an unchanged previously-cleared verse — appropriately NOT normalized. **TERMINATION BAR met → GG witness 1ki2 APPROVED & IMMUTABLE** (content final at `c3db778`). Full convergence: R1 (1 crit + systematic) → R2 (1 crit + bounded) → R3 (9, exposing a round-2 over-correction) → **R4 clean**.

**⏭ RESUME HERE → `1ki2` C-4 (acquire CAM hi-res):** the GG witness is APPROVED & IMMUTABLE (content final `c3db778`; do NOT re-open or re-transcribe it). **C-4 (controller):** from the just-approved GG 1Ki2 narrative + the CAM low-res locating band `GAPS/2_Kings/Cambridge-Add-1570/Kings_f126r…f146v.jpg`, identify the CAM Add.1570 folio(s) for 1Ki2 **by VISION** (the IIIF ToC `structures` MISLABEL the Ethiopic Reigns — never trust the label; 1ki1 CAM was f126r→f126v at views around the f126 band, so 1ki2 immediately follows — confirm by vision against the GG narrative, do not assume). Pull from CUDL IIIF per memory `cudl-iiif-access`: fetch manifest `https://cudl.lib.cam.ac.uk/iiif/MS-ADD-01570`, find the view, region-tile the ~80 MP master in ≤1900 px tiles `/{x},{y},{w},{h}/full/0/default.jpg` (single-delivery caps 1503×2000), stitch with Pillow, QC the stitch (legible at native zoom; full chapter present; folio identity double-confirmed against the f106r=view215 anchor + the manifest label, zero drift). Save `GAPS/2_Kings/Cambridge-Add-1570-hires/MS-ADD-01570_f{NNN}_1ki2_hires.jpg` (CC BY-NC, "Cambridge University Library"). **ALL scratch (manifest JSON, IIIF tiles) → an OS temp dir OUTSIDE the repo** (`C:\Users\bogda\AppData\Local\Temp\…`); controller `git status --short` gate (zero stray scratch) before any checkpoint. Then **C-5** isolated-opus blind CAM transcribe of the hi-res image(s) ONLY (no GG, no GG JSON, no skeleton) → `content/manuscript/kings/calibration/1ki2_witnessCAM_hires.json` (SHIPPED schema; both METHOD NOTES — anti-harmonization CARDINAL RULE + column/folio-boundary make-or-break; ignore CAM side-margin marginalia per spec §2) → **C-6** adversarial CAM review + fix-loop to convergence (validator `OK`, screen `CLEAN`, boundary make-or-break on every claimed minus) → on APPROVE CAM witness immutable + checkpoint commit → **C-7** collate via the shipped Phase-2 tool (`mc.load_kjv_skeleton('1ki',2)`, `mc.collate(...,book='1ki',chapter=2)`, `mr.reconcile`, write `content/manuscript/kings/collation/1ki2_collation.json`) → **C-8** adversarial collation review (independently recompute every metric from `alignment[]`; lacuna == evidence `⟦illegible⟧`; token-conservation; base + semantic) + fix-loop → **C-9** fill manifest `1ki:2` (GG.folios=f029r/f029v/f030r + source_images; CAM.folios/views from C-4; `status: calibrated`) + `./save.cmd "tau.6.x.4.c 1ki2: blind dual-witness GG+CAM transcription + collation (semantic NN/NN, bothW NN%, base=CAM); manifest calibrated"`. **NO C-10** (1ki1-only safety-stop; the model is ratified — marathon continues continuously, NO user check-in). Then the next queued chapter `1ki3` (full C-1…C-9 verbatim), … `1ki22` → `2ki1` … `2ki25`. This is the explicitly **multi-session** marathon body — the manifest (`run_manuscript_collation_at_scale.py --track kings`: total 47 / collated 1 / pending 46; `1ki:2` `pending` until its C-9) is the durable cross-session ledger; resume from the first `pending` chapter. After all 47 `calibrated`: S1-QA, then Stage 2 = Phase-3 render `geez-tewahedo/1ki.py`+`2ki.py` + apparatus + `manuscript-collation-tier2`. Local commit only — no push, no zip.

---

---

## Closed/historical arcs (lines 157+ of pre-trim IN_FLIGHT.md)
