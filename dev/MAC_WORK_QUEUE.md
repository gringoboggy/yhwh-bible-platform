...
- Report your current rules for autopulling and continuing work when done (extract from your scripts/lane_watch_mac.sh, agent_idle_radar, memory, .claude/config if any). Format symmetrically with WIN's (see LANE_HANDOFF NEVER-STOP and checklist). Send via LANE_HANDOFF update or report. WIN will format for both and mirror back.
