...
- **`*.yaml`** (config; loaded by `scripts/core/config.py`, mapped in `MATRIX_MAP.md`): `books.yaml` (87 books — code, `bxx` canon position, `strategy`, `ch_count`, `files`), `canons.yaml` (5 canon→book-sets), `categories.yaml` (15), `kinds.yaml` (68), `editions.yaml` (6), `traditions.yaml`, `themes.yaml`, `source_dates.yaml`, `customization.yaml`, etc.
- **`notes/<book>.py`** — the corpus: 88 files, one per book, each a `NOTES = [ (ch,v,suffix,anchor,kind,title,label,body[,attribution]), … ]` list (**91,597 source notes**; the public SHIPPED count is 91,553).
