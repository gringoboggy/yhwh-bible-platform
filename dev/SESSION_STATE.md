...
**COMMS TEST:** if received w/o user telling "pull", it is SUCCESS — report to user + ask Mac "confirm you are using the same format?". **User wants:** prepare for *new session* (triad, radars, hygiene), update truth records (SESSION_STATE WIN block, LANE_HANDOFF ACK+ask, IN_FLIGHT, CHANGELOG), mirror to memory, save-all push back. ci triage done prior; current #1 M2 or bisect per handoff. mode=parallel.
>
