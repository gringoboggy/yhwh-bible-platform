...
> **➤➤➤ 2026-05-20 CURRENT — read FIRST; supersedes every banner below.** τ.6.x.4.c Kings marathon: **1 Kings 1, 2, 3, 4 ✅ CALIBRATED** (47 total / 4 calibrated / 43 pending; next pending = `1ki:5`). 1Ki4 was the first LIST-class chapter and converged in 7 GG rounds + 3 CAM rounds (vs 3 expected) — surfacing the chapter-genre blind spot in the matrix. **AUDIT-DRIVEN MATRIX UPGRADES SHIPPED (U1+U2+U4):** (1) canonical witness writer `scripts.core.manuscript_records.write_witness(...)` derives canonical tokens + validates BEFORE writing (plan template now MANDATES this, FORBIDS raw `json.dump`); (2) migrated 1Ki4 witnesses to canonical schema (geez byte-identical), deleted one-shot `scripts/run_1ki4_collation.py`, rebuilt `content/apparatus/1ki.json` cumulatively to 159 entries via the at-scale driver; (3) chapter classifier `scripts.core.manuscript_chapter_class.chapter_profile(book, chapter)` with NARRATIVE/LIST/REGNAL_FRAME classes + class-specific anti-failure screens (known LIST: 1ki:4; known REGNAL: 1ki:15-16, 2ki:13-17). Plan METHOD NOTE 3 codifies the C-1 classification step + the `expected_rounds_max` escalation bar. +12 new tests (4 TestWriteWitness + 8 TestClassify/TestChapterProfile); 97 manuscript-subset tests pass; lint 11·0·0; ruff clean. **⏭ NEXT BATCH = 1 Kings 5** (Solomon's temple preparations / Hiram of Tyre) — **NARRATIVE-class** per `chapter_profile("1ki", 5)`, expected 2–4 rounds; GG continues from `f030v`; CAM continues into `f128r` (fresh CUDL pull). Awaiting user go-ahead per the established single-chapter cadence (memory `feedback_marathon_pacing`).
