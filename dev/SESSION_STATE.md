...
> **➤➤➤ FRESH SESSION START HERE (🪟 WIN, turn 105).** `/resume` → pull @ `e4dd1a5d` → **fix 11 pytest reds** (list below) → spot re-run `pytest -m "not slow"` → Phase 3 audit fixes (`build_edition.py` / `generate_verse_popups.py` / `config.py`). **Audit2 DONE (turn 105 prep):** full `not slow` pytest @ pre-`e4dd1a5d` tree — **8081 passed · 11 failed · 262 errors** (3h34m); 262× `api_select_book_cover` ImportError = pytest started before covers commit landed — **gone @ `e4dd1a5d`**; re-run expected ~12 reds max. **11 failures:** `test_build_standalone_produces_epub` · `test_at_scale_base_is_the_single_candidate_json_shape_source` · `test_green_when_display_none_physically_stripped` · `test_preflight_exposes_manuscript_check` · `test_chapter_entry_track_aware` · `test_chapter_last_verse_badge_stays_in_its_chapter` · `test_build_edition_passes_ruff_check` · `test_every_chapter_has_both_witness_folios[kings]` · `test_do_GET_routes_cache_status_through_dispatch_helper` · `test_dispatch_loop_reads_body_once` · `test_do_POST_dispatches_to_multipart_table`. Also watch `test_work_cache.py::test_persists_to_disk` (ERROR). One job at a time. Baton **windows** (truth_owner); mode=parallel.
>
> **➤➤➤ Mac (turn 105 — parallel, disjoint).** **INGEST v4 book title covers** — `git pull` @ `e4dd1a5d` (or `E:\YHWH-v2.4-releases\book-title-covers-v4-reimagine-2026-06-16\` per `MAC_INGEST.md`) → audit ×3 · `test_book_title_covers` · wire `ethiopian-tewahedo` `book_covers` · `/covers` A/B/C smoke. Phase 4 disjoint still queued. **HOLD:** `build_edition.py` · `web.py` until WIN pytest green.
>
