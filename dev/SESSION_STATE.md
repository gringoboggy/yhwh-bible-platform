...
> **➤➤➤ 2026-05-19 CURRENT — read FIRST; supersedes every banner below for the LIVE resume point + cadence.** τ.6.x.4.c Kings marathon: **1 Kings 1 & 2 ✅ CALIBRATED & committed** (1ki1 `9bb9976`; 1ki2 `40fd8a8` — incl. the engine base SURFACE-TO-USER event, resolved to **base=CAM per the ratified 2026-05-17 decision-of-record** as a segmentation-granularity artifact: token-extent CAM/GG 1.033, semantic 46/46, C-6 converged 3 rounds, C-8 token-conservation exact; full record `dev/CALIBRATION_2026-05-19-kings-1ki2.md`; witnesses immutable GG `c3db778` / CAM `b0d939e`). **1 Kings 3 IN PROGRESS:** C-1 locate ✅ (entirely on GG folio **f030r**, L→M→R); **C-2 blind GG transcribe ✅ controller-verified** (`content/manuscript/kings/calibration/1ki3_witnessGG.json`, 27 v contiguous, 455 tok, validator OK, screen CLEAN, 1 honest `⟦illegible⟧` col-L blot bijection-OK; **WIP-committed this checkpoint, MUTABLE until C-3 approves** — manifest `1ki:3` still `pending`). **⏭ RESUME = 1 Kings 3 C-3** (adversarial GG review + fix-loop) → C-4 acquire CAM hi-res (1ki3 CAM begins f127r-M per 1ki2 C-4; f127r hi-res already on disk) → C-5 → C-6 → C-7 → C-8 → C-9. **NO C-10** (1ki1 only). **⚑ CADENCE CHANGE (user, 2026-05-19): run in SMALL BATCHES of ~2–3 chapters, then STOP and report for an explicit go-ahead before the next batch. The "continue continuously, no user check-in" default below is SUPERSEDED** (memory `feedback_marathon_pacing`). A base SURFACE-TO-USER (engine §3.3 clause-3) or any CONTRADICTS trigger still stops immediately regardless of batch position. Manifest = durable cross-session ledger: **47 total / 2 calibrated / 45 pending** (next pending = `1ki:3`). Historical banners below retained for arc history; the live resume point + cadence are HERE.**
>
