...
> **➤➤➤ 2026-06-15 (🪟 Windows, turn 94 — ★ K-R15a FIX + ROUND-16 KEPUB ON KOBO).** **K-R15a follow-up:** `badge-trail` invisible chars no longer block empty-verse repair — Gen 8:15 now shows KJV fallback prose on device. **QA loaded:** `G:\YHWH-koboQA.kepub.epub` (`…T234655Z`). **Audit READY (do not push until merge):** parallel round-8 runbook `docs/superpowers/plans/2026-06-15-round8-parallel-audit-session.md` — Win 7 dims + Mac 14 dims, FINDINGS-ONLY. **Deferred next ship:** EPUB colour (ToC/pills/matter); per-book cover art refresh (`content/covers/_book_defaults/` + 21 Ethiopic extras); translation popup letter-spacing. Baton **mac**; mode=parallel. TRACKER-STATE active.
>
