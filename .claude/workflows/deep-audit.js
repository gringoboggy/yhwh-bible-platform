...
const ROUND = args?.round ?? 16            // args don't reliably propagate — bump in-file (round-16 build-program bulletproofing audit)
const NOW = args?.now ?? '2026-06-27'             // Date.now() is unavailable in scripts; stamp via args
...
const DEFERRED_BY_DESIGN = args?.deferred ?? [
  ...
  // ---- ROUND-15 D1–D9 SETTLED / SHIPPED (program dev/audit/round-15-completeness-audit-program-2026-06-26.md; round-16 must NOT re-litigate) ----
  'ROUND-15 D1: release-asset integrity is SHIPPED — dev/audit_release_assets.py + the live v0.1.0 re-cut (100 retired-SKU EPUBs + their SHA256SUMS lines removed → 87 assets, bijection clean). Do NOT re-flag retired-SKU release assets, orphan/missing sum lines, or the release bijection.',
  'ROUND-15 D2: the xref subsystem is SHIPPED — G3 audit_idmap_frags xref-class breakout (scripture v-/ch- counted+bucketed separately) + a --min-xrefs 10000 floor wired into the per-build gate (ethiopian 88,541 / catholic-study 55,774 xrefs, 0 dead) + the check_xrefs.py:52 regex guard. Do NOT re-derive the xref counts or re-flag check_xrefs.py:52.',
  'ROUND-15 D3: Douay/Clementine-Vulgate Psalm 2:13→2:12 + 4:10→4:8 are FIXED (_VULGATE_PSALM_FIXES) via a surgical epub_working/ re-bake + a golden re-stamp; the restored Latin tails SHIP. Do NOT re-flag the dropped psalms or re-litigate the bake. (The generate_verse_popups base↔bake separator-drift is a known standalone follow-up, not a round-16 finding.)',
  'ROUND-15 D4: the /customize note-flag cascade is FIXED — _cascade = s2_group (build_edition.py:4228); dict-easton provenance restored 0→545 in the {S1-on,S2-off,eink} combo (byte-safe for shipped editions). Do NOT re-raise the S1/S2 attribution combo.',
  'ROUND-15 D5: the flagship study-glossary byte-streamer is PROVEN — _stream_glossary_pieces_from_bytes str==from-file byte-identical 690/690 on the real 255 MB index_split_900 monolith + G5 PASS on the fresh flagship eink build + a permanent real-threshold regression. Do NOT re-diagnose the glossary streaming or re-derive the OOM (the residual ~3×-copy RAM surface is a NEW resource finding for the round-16 builder-robustness dim, not a re-raise of D5).',
  'ROUND-15 D6/D7/D8/D9 SHIPPED: D6 canon book-count gate (audit_canon_bookcount) · D7 migration torn-safe + idempotence gate · D8 audit_canonical_order reading-flow order gate (validated 1299 ch / 75 books, no defect) · D9 kepub bare -sN rev-id family CONFIRMED by-design (rev navigate ids bare on purpose; inline -sN present). Do NOT re-litigate any of these; round-16 cross-module reuses audit_canonical_order, it does not re-derive D8.',
  // ---- ROUND-16 SCOPE NOTE: the round-14 build-source dims + est 10:2 are SETTLED → regression/bleed watch ONLY ----
  'ROUND-16 SCOPE: the 6 round-14 build-source settlements (file-split idmap-miss fallback, eink-gating isolation, WS2 cascade de-dup, glossary streaming, the page-break re-arch, WS3 popup separators) AND the est 10:2 verse-displacement fix (_mv_displacement_would_corrupt) are SETTLED — the round-16 new dims audit them for REGRESSION/BLEED only; a verifier MUST refute a finding that merely re-raises the original design or re-derives the original counts.',
]
const PRIOR_SURVIVOR_TITLES = args?.priorSurvivors ?? [  // round-15 fixes — confirm-not-regress (program priorSurvivors)
  'D1 release-asset integrity gate (audit_release_assets.py; v0.1.0 re-cut to 87 assets)',
  'D2 G3 xref-class breakout + --min-xrefs 10000 floor in the per-build gate',
  'D3 Douay/Vulgate Ps 2:12/4:8 restored (epub_working re-bake + golden re-stamp)',
  'D4 _cascade = s2_group (build_edition.py:4228; dict-easton provenance restored)',
  'D5 flagship glossary byte-streamer proven 690/690 @255MB + G5 PASS + regression',
  'D8 audit_canonical_order reading-flow order gate (1299 ch / 75 books)',
]
...
Hunt for: (a) RULE DUPLICATION — the same rule stated in 2+ homes (RULES vs PLAYBOOK vs memory topic files vs LANE_HANDOFF boards) where the copies have DRIFTED or could drift (cite both homes + the divergence); (b) CONTRADICTIONS — two rules/doctrines that conflict (cite both); (c) TOKEN BLOAT in every-session reads — sections of the triad/hooks output that are history rather than live instruction and should move to dev/archive/ (the triad is ~700-900 lines BY DESIGN, so flag only genuinely dead/duplicated weight, with byte estimates); (d) HOOK CORRECTNESS — stale file names, broken paths, hooks that no-op silently, SessionStart output that misstates the live state; (e) SETUP SECURITY — secrets or tokens in settings/hooks/memory files (report PRESENCE + LOCATION ONLY — never quote a secret value into a finding), overly broad permission allowlists, hooks executing untrusted input. Do NOT flag the existence of the memory/hook system itself (it is deliberate); flag defects IN it.`,
  ...
  },
  {
    ...
    prompt: `DIMENSION: POPUP / ASIDE INTEGRITY — the K-R4 "nowhere else" class sweeps (prior art: ${REPO}/docs/superpowers/notes/2026-06-10-kobo-round4-device-qa.md — READ IT FIRST; do NOT re-derive K-R4-1/K-R4-2 themselves, EXTEND beyond them). The v0.1.0 dist artifacts were removed — BUILD A FRESH ethiopian eink .epub + .kepub.epub to inspect (the aged-asset trap; never scan a stale dist artifact). You may run Python zip-scans via Bash (interpreter: py -3 with $env:PYTHONUTF8="1").
...
Also check the build code for emitters added later that would silently miss the separator/clamp/size policies (the fix-the-class rule). Findings need the emitter file:line or the artifact ids + counts.`,
  ...
  // ---- ROUND-16 BUILD-PROGRAM BULLETPROOFING DIMS (2026-06-27; program dev/audit/round-16-build-program-bulletproofing-2026-06-27.md).
  //      The full output cross-product (edition × reader-version × OS) + options-completeness + display-redundancy +
  //      HTML-integrity + per-reader marker-logic + builder-robustness lenses. ALL read-only SOURCE analysis (model-bound
  //      → BOTH lanes). The WIN box ALSO runs a full-catalog build-inspect harness concurrently (it parses the BUILT
  //      artifacts; these source dims find the CODE sites that produce the defect). ----
  {
    key: 'builder-robustness', kind: 'find', finders: 2,
    prompt: `DIMENSION (round-16, user ask #5 "the builder must not crash"): BUILDER RESOURCE / PROCESS ROBUSTNESS — RESOURCE/PROCESS failure ONLY (logic defects belong to 'correctness'; injection to 'security' — do NOT double-report a crash in all three). In ${REPO}/scripts/build_edition.py + scripts/core/* hunt for: (a) OOM — the RESIDUAL study-glossary slicing that holds ~3× copies of the body (the offset-based slice fix was recommended but NOT done: _split_head_body_tail / _study_index_section_parts create text[start:end] slice copies while the parent text is still referenced; _iter_study_glossary_pieces keeps its \`text\` param alive for fallback yields) → the flagship-eink RAM ceiling; (b) a non-atomic / torn write into epub_working/ or the per-edition working copy (a write NOT routed through notes_io.atomic_write that an interrupted run leaves half-written); (c) any subprocess.run / Popen missing stdin=subprocess.DEVNULL (Windows WinError 6); (d) frozen-vs-dev divergence — a path/behavior that differs under sys.frozen (the build_epub in-process-vs-subprocess fork ~8558) that could crash ONLY in the PyInstaller binary; (e) an unguarded MemoryError / no memory-pressure backoff in the all-9 ThreadPoolExecutor(max_workers=5). Cite file:line + the exact failure trigger. SETTLED (do NOT re-diagnose): the A2 single-pass _apply_splices + the round-15 byte-streamer (proven 690/690 @255MB) — find a NEW resource/process failure, not a re-raise.`,
    angles: ['Emphasize the residual glossary 3×-copy OOM + the offset-vs-slice opportunity + the ThreadPool memory ceiling.', 'Emphasize non-atomic/torn writes, subprocess DEVNULL, and frozen-vs-dev path/behavior divergence that crashes only the binary.'],
  },
  {
    key: 'cross-product', kind: 'find', finders: 2,
    prompt: `DIMENSION (round-16 HEADLINE, user asks #3 + #8): CROSS-PRODUCT GRID PARITY — the product is offered as (edition × format/target_reader × OS-binary); prove the GRID is COMPLETE + internally consistent. This is NOT the per-cell output quality (that is html-integrity/whitespace/marker/display). Read ${REPO}/scripts/build_edition.py (FORMAT_MATRIX ~2034, TARGET_READERS ~1977, resolve_target_reader/apply_target_override/is_kindle_target ~1980-2017, COVER_COLOURS ~2092, catalog_asset_name), scripts/build_format_matrix.py, scripts/templates/customize.py (the reader dropdown), content/editions.yaml. Hunt: (a) MATRIX-DECLARED ≠ BUILT — the FORMAT_MATRIX \`kindle\` row declares target_reader:"kindle" but the shipped recipe is the everywhere base + kindle_post.make_kindle_safe (the --target-reader kindle variant was retired); assert declared==actually-built for every cell. (b) ORPHAN OPTION — \`computer\` is a valid TARGET_READERS value AND a /customize reader option but has NO FORMAT_MATRIX row → a silent alias of everywhere with no catalog asset + no test (ask #8 "all options logical and present"). (c) \`play\` = everywhere (provisional/M5) — intended alias or a missing profile? (d) STANDALONE degeneracy — apply_target_override RAISES on a standalone given --target-reader (~2005); confirm geez/amharic get NO target (one shape each) and nothing iterates a 4-reader cross-product over them. (e) cover-swap is LOGIC-NEUTRAL (swap_epub_cover.py touches only cover assets, never content/markers). (f) phase-gating (M1/M3/M4/M5) coherent; no orphan/missing/duplicate (edition×format) cell vs the catalog. Cite file:line + the inconsistent cell. PAIRS with the build-free gate dev/audit_cross_product.py.`,
    angles: ['Emphasize matrix-declared≠built (kindle row) + the `computer` orphan option + the `play` alias question.', 'Emphasize standalone target degeneracy, cover-swap logic-neutrality, and phase-gating / no-orphan-cell completeness.'],
  },
  {
    key: 'marker-logic-xreader', kind: 'find', finders: 2,
    prompt: `DIMENSION (round-16, user ask #4): TRANSLATOR + STUDY-NOTE MARKER LOGIC, PER READER. The two marker families: the translation vn-link→vnote popup (verse START) and the study-note marker (marker_style ∈ {badge, numbers}). Prove each is internally consistent + COMPLETE on EVERY reader profile (everywhere/tablet/eink/kindle), in ${REPO}/scripts/build_edition.py: apply_badge_markers (~4006 — badge collapse + aside merge), the numbers path (inject.build_marker/build_aside + renumber_markers), _apply_popup_languages_and_translation, drop_orphan_vnote_asides, inject_eink_study_backmatter, and the kindle_post endnote presentation. Hunt: (a) an ORPHAN marker — a badge/number/vn-link whose href target id is absent in the piece the reader lands in (no popup → dead tap); (b) a NOTE with no reachable marker (note shipped, marker dropped); (c) badge count ≠ pre-collapse marker count (badges_skipped is tracked only in a stats sidecar — surface any nonzero path); (d) the NUMBERS marker_style — G4 badge-conservation does NOT cover it, so prove numbers editions have gapless, target-resolving markers too; (e) per-reader correctness: tablet keeps verse-end badge + verse-start vn-link, eink routes study notes to glossary backmatter (badge→glossary jump), kindle SUPPRESSES popups → visible endnotes (display:none stripped) — assert each reader's treatment is the intended one + complete, no marker→nowhere, no teleport (the known STK m4b vn-link teleport is a DEVICE/KFX issue — confirm in/out of code scope, do NOT re-derive it). Cite file:line or the artifact id + the reader. PAIRS with the harness scanner + the extended G4.`,
    angles: ['Emphasize orphan markers, note-without-marker, and badge-count≠marker-count across readers.', 'Emphasize the numbers marker_style (G4-uncovered) + per-reader treatment correctness (tablet/eink/kindle) + no teleport.'],
  },
  {
    key: 'html-integrity', kind: 'find', finders: 2,
    prompt: `DIMENSION (round-16 HEADLINE, user ask #2): HTML WELL-FORMEDNESS + NO CODE/TEMPLATE LEAK in emitted output. The headline NEW gap: there is NO gate today for code/template leak in built HTML, and the [Reviewer:] scaffolding lint runs COMMIT-TIME only (never re-verified on the built artifact). In ${REPO}/scripts/build_edition.py + scripts/inject.py + scripts/core/{html_sanitize,html_sandbox,popup_versions}.py + scripts/templates/*.py + the matter_pages render_* family, hunt for any path that can emit into shipped HTML: (a) an UNRENDERED template token — a literal {{...}} / {%...%} / \${...} / a Python repr like ['  or {'  / an f-string artifact (a stray { from a mis-escaped brace) — find every f"...{...}..." / .format() / template-substitution site that interpolates note/verse/title/attribution text WITHOUT escaping; (b) an UNBALANCED or MALFORMED tag — a <tag> opened and not closed, a broken </ >, a stray raw < or > from un-escaped note-body text (sanitize_html DROPS tags = silent content LOSS, not just XSS); (c) a NESTED <a> reintroduced by a post-pass AFTER the base check_nested_anchors ran (G1 checks the BASE only — per-edition builds never re-verify); (d) the scaffold-strip regex over-greedy (eats real body) or under-greedy (leaves [Reviewer:]). Fix-the-class: grep EVERY interpolation/emit site, state how many you checked + which. This dim finds the SOURCE sites; the WIN harness scanner dev/audit_output_hygiene.py proves it on the built artifact. Cite file:line + the leak class.`,
    angles: ['Emphasize template/code-leak: every unescaped interpolation site that can emit {{}}/{%%}/repr/f-string artifacts or a raw < >.', 'Emphasize tag well-formedness: unbalanced/malformed tags, a post-pass-reintroduced nested <a> (the G1 base-only blindspot), and over/under-greedy scaffold strip.'],
  },
  {
    key: 'whitespace-pagebreak', kind: 'find', finders: 1,
    prompt: `DIMENSION (round-16, user ask #6): NO unwanted page-breaks, empty spaces, or line-breaks. In ${REPO}/scripts/build_edition.py read: apply_empty_verse_prose_repair (the empty-verse repair has NO verification gate today), _merge_mid_verse_breaks (~5724) + _MIDVERSE_BREAK_KEEP_BOOKS, _merge_scripture_base_files (~5505, eink per-book contiguity — a NEW SPINE FILE is the only Kobo page break), apply_file_split (chunk boundaries), apply_eink_verse_line_breaks, apply_vnote_preview_separators / apply_tablet_popup_sep_stripped (separators visible-on-eink / stripped-on-tablet). Hunt: (a) an empty verse body that ships un-repaired (a marker-only verse after a filter drop) — plus the fact the repair has no post-check; (b) an empty <aside>/popup (all rows filtered out but the shell + badge remain → a tap opens nothing); (c) a stray <br> / blank line / orphan chapter-opener left at a file-split piece boundary; (d) an UNWANTED spine/page break — a piece cut that forces a Kobo page break mid-content, or a merge that fails (returns 0, abort-to-noop) and SILENTLY ships a half-merged spine with seams; (e) tablet separators NOT stripped (stray dashes) or eink separators leaking into non-eink output. The kept poetry/wisdom mid-verse breaks are BY DESIGN (do NOT flag). Cite file:line or the artifact id + the reader. PAIRS with dev/audit_output_hygiene.py (it reuses audit_spine_breaks).`,
  },
  {
    key: 'display-redundancy', kind: 'find', finders: 1,
    prompt: `DIMENSION (round-16, user ask #7): NO REDUNDANCY in what the badge/popup/translation/study-note display shows OR in how it shows it. In ${REPO}/scripts/build_edition.py read: _emit_cascade_sections (~3896 — category→source→note grouping), _strip_redundant_note_label + _strip_redundant_body_boilerplate (gated on note_attribution_dedup), apply_badge_markers's cross-verse byte-identical de-dup (~4214), the glossary backmatter. Hunt: (a) a REPEATED category header or attribution byline across cascade sections — the dedup is PER-VERSE only; there is NO gate on S2-cascade header/byline repetition (a category heading or an "Easton's Dictionary" byline printed twice in one popup/section); (b) the SAME note body shown twice (a de-dup that MISSES) OR, conversely, a de-dup that drops a DISTINCT note (= data LOSS) — flag BOTH directions; (c) a duplicated badge or a duplicate translation witness (the same version's text rendered twice in a verse popup); (d) redundant MARKUP — duplicate wrapper divs, repeated CSS-class spans, an empty label span left behind. Regression-watch ONLY on the round-14 WS2 cascade de-dup (SETTLED — do NOT re-litigate; flag only a NEW redundancy class). Cite file:line or the artifact id + what is shown twice. PAIRS with dev/audit_output_hygiene.py.`,
  },
  {
    key: 'options-completeness', kind: 'find', finders: 2,
    prompt: `DIMENSION (round-16, user ask #8): ALL USER-FACING OPTIONS LOGICAL + PRESENT + WIRED END-TO-END. The §7 wiring contract (RULES §9 / MATRIX_MAP) = each editions.yaml field flows: field → config LOADER → enum VALIDATOR (api_save_edition_meta) → /customize CONTROL (scripts/templates/customize.py) → BUILD READ (build_edition) → TEST. Read scripts/templates/customize.py, scripts/api/*.py (customize/editions save+load), content/editions.yaml, build_edition.py. For EVERY option (marker_style, marker_badge_style ×8, note_popup_style {chip,pills,category-color}, verse_popup_style {cards,stack}, note_attribution_dedup, note_group_by_category, note_topic_dedup, note_popup_split_cap/byte_cap, topical_index_source {both,naves,torrey}, per-book/per-chapter symbol overrides, traditions, theme, title_page_style, chapter_number_format, target_reader, popup_translation, cover_template) hunt: (a) a field MISSING one of the 5 wiring legs (a control with no validator, a validator with no build-read, a field with no test) → an option that silently no-ops or accepts an invalid value; (b) an enum value UNREACHABLE from the UI or that the build does not handle (a dead/illogical choice) — esp. the \`computer\` target_reader orphan (offered, no build branch); (c) an option whose two values produce IDENTICAL output (a meaningless toggle); (d) a default that disagrees between editions.yaml-unset and the resolver. Build-free (no EPUB). Cite the field + the missing leg / unreachable value. PAIRS with dev/audit_customize_completeness.py.`,
    angles: ['Emphasize the 5-leg wiring completeness per field (a missing validator/build-read/test) + the `computer` orphan.', 'Emphasize unreachable enum values, meaningless toggles (identical output), and default disagreements.'],
  },
  {
    key: 'os-binary-parity', kind: 'find', finders: 1,
    prompt: `DIMENSION (round-16, user ask #3 "Windows/Mac/Linux programs as a whole"): OS DESKTOP-BINARY PARITY. READ-ONLY static analysis (do NOT build the binaries — out of N95 scope; the 3-way binary truth is the build-linux.yml CI + local Win/Mac). Read ${REPO}/dev/launcher.spec (shared by all 3 OSes), scripts/core/paths.py (content_root / sys.frozen / YHWH_DATA_DIR), the launcher entry, dev/build_desktop.{cmd,sh}, dev/build_msi.cmd + dev/installer.iss (Win), dev/build_dmg.sh + dev/YHWH.entitlements (Mac), dev/build_appimage.sh + .github/workflows/build-linux.yml (Linux), dev/sign_windows.ps1. Hunt: (a) CONTENT/VERSION DRIFT — the 3 builders must bundle the SAME content/ + editions.yaml + VERSION; any OS reading a different path or a stale version string; the launcher.spec datas excludes (.backups/) consistent across OSes; (b) a paths.py / sys.frozen / content_root resolution that works in dev but is WRONG in the frozen binary on one OS (the round-12 frozen content_root class); (c) the webview import NOT in a try/except → a headless/Linux build crashes instead of falling back to browser mode (build-linux.yml runs without pywebview); (d) SECRET LEAK — a signing/notarization credential echoed or committed in sign_windows.ps1 / build_dmg.sh (report PRESENCE + LOCATION only, NEVER the value); (e) installer/dmg/AppImage chain soundness (staple step present, checksum coverage). Cite file:line + the per-OS divergence. Do NOT flag CSRF/hosting (single-user local app).`,
  },
]
...
// ROUND-16 build-program bulletproofing (2026-06-27): the full output cross-product (edition × reader-version
// × OS) + options-completeness + display-redundancy + HTML-integrity + per-reader marker-logic lenses. All
// read-only SOURCE analysis (model-bound) → BOTH lanes run the SAME set (LANE only picks REPO + agent types;
// do NOT partition dims per box — that caused the round-5 15× Mac failure). WIN additionally runs the
// full-catalog build-inspect harness; Mac additionally cross-OS-verifies. 'tests-run' OMITTED (never pytest
// beside a build). The 6 round-14 build-source dims are SETTLED (in DEFERRED_BY_DESIGN) → regression/bleed only.
const ROUND16_DIMS = new Set(['correctness', 'security', 'builder-robustness', 'cross-product',
  'marker-logic-xreader', 'html-integrity', 'whitespace-pagebreak', 'display-redundancy',
  'options-completeness', 'os-binary-parity', 'cross-module'])
const _laneSet = LANE === 'all' ? null : new Set(LANE_DIMS[LANE] || [])
const DIMENSIONS = args?.dimensions ?? (ROUND === 16 ? _scoped.filter((d) => ROUND16_DIMS.has(d.key)) : (ROUND === 14 ? _scoped.filter((d) => ROUND14_DIMS.has(d.key)) : (_laneSet ? _scoped.filter((d) => _laneSet.has(d.key)) : _scoped)))
