...
- **`source_key(attribution)`** (the canonical grouping key, also the basis of the display byline):
  ...
  2. strip a trailing citation locator on per-locator commentary — chapter/verse markers `\b[IVXLC]+\.\d+\b` together with any preceding structural word `Bk`/`Book`/`Hom.`/`Homily` (SK-2: e.g. Ephrem `Commentary on Genesis I.11`, Cyril `Commentary on John, Bk I.11`) and the series tail (`NPNF…`, `vol\.\s*\d+`, **looped to a fixpoint** — POLISH-1, so `…NPNF Series 2, vol. 13` strips whole) — so per-locator commentary from one author+work collapses to ONE byline rather than one-per-locator, with no dangling `Bk`/`NPNF Series N` fragment;
  ...
  - *Verified safe for grouping (SK-4, re-verified 2026-06-09):* across all 1,043 distinct attributions → 790 keys, **22** keys collapse >1 attribution. **19** are comm-ethiopian sources whose group byline is **suppressed** (self-attributing bodies — §9), so they never render a shared byline. The remaining **3 RENDERING** collapses are all intended/lossless: the Strong's Hebrew dictionary (40 numbered entries → one key) and the Strong's Greek dictionary (59 → one key) — lossless because the headword lives in the body — plus one `Alter`/`alter` casing pair. Notes with empty/`None` attribution group under one unattributed bucket (no byline).
