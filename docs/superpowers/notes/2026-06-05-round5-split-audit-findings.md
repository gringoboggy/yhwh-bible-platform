...
- [ ] **(HIGH) `core/edition_stats.py` absent from `_PIPELINE_SCRIPTS`** — `scripts/core/build_cache.py:62-81`. `matter_pages.py` embeds `resolved_note_counts()` output into `copyright.xhtml`/`symbollegend.xhtml`/`your-edition.xhtml` (packed into the EPUB), but editing `edition_stats.py` logic doesn't bust the cache → a **stale EPUB** is served. **Fix:** add `"core/edition_stats.py",` after line 80 (documented comment; same pattern as popup_versions/traditions mint-10, source_dates mint-11 #22). Also add `"core/book_native_names.py",` (dormant, free now). **Then evict:** `build_cache.cache_clear()` / delete `exports/.cache/`. **Byte-stability obligation:** regen 9 KJV + `git diff` (expect zero) + `test_byte_stability_gate.py`. **Guard to add:** a `lint_rules.py` commit-time check that every `scripts/core/*.py` imported by build-path modules appears in `_PIPELINE_SCRIPTS` (or is allow-listed) — cache-coverage drift recurs every new build-path module.
...
- [ ] 🪟 **(LOW) `apply_badge_markers` silent orphan-marker bail** — `build_edition.py:1818, 1904-1912, 4207-4210`. Additive stats: `"badges_skipped": 0` init + increment before `continue` + propagate + optional `logger.warning`. Byte-stable (stats not in EPUB); confirm via `test_byte_stability_gate.py`. Guard: a fast unit test asserting `badge_verses_skipped > 0` on an orphaned-badge edition.
...
- 🪟 **`filter_html` per-kind regex loop** (`build_edition.py:1332-1346`) — **CHANGE (LOW):** combine `disabled_kinds` into ONE marker alternation + ONE aside alternation, pre-built once per edition in `build_one`, passed as optional kwargs (fallback to in-function compile). ~70-80 scans/file → 2. Per-element classes mutually exclusive → byte-identical. **Mandatory** `test_byte_stability_gate.py` + 9-edition `git diff` = zero before merge. Do NOT collapse the `disabled_html_ref_ids` loop.
