...
- [ ] **Step 3: Implement** `scripts/manuscript_qa.py` (rules §9 `run_all()` shape: `{"checks":[{id,name,status,message,violations}],"summary":{total,pass,warn,fail,clean}}`; `main()` → 0 clean / 1 on any `fail`). Concrete check ids:
  ...
  - `engine_metric_<ref>` for each of `1sa1`/`1sa3`/`1sa17`/`2sa11` — the engine's OWN computed metrics vs the design-spec §4 reference bar: **`fail`** if `semantic_pass_pct < 95` OR `uncertainty_pct > 10`; **`warn`** if the engine's W↔W (`ww_agreement_bothconfident_pct`) `< 90` (EXPECTED for distinct recensions — diplomatic-parallel; explicitly NOT a `fail`); else `pass`. Message states the three numbers + "W↔W sub-bar is expected (distinct recensions; GO already given)".
  ...
  Fold into `_compute_preflight_uncached()` in `scripts/web.py` per rules §9 step 3: try/except-guarded (failure → `warn`, never 500), additive, `jump_to:"/preflight"`, id contains `"manuscript"`.
