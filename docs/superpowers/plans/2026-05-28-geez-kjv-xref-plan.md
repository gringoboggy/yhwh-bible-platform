...
- [ ] **Step 3 — implement `build_kjv_xref(collation, kjv_rows, book) -> dict`.**
  ...
  - **Monotonic filter:** keep the **max-total-score** non-decreasing subsequence of `cand` by base order (score-weighted O(N²) DP — drop anchors that violate monotonicity AND prefer the chain with highest combined evidential weight; a length-maximising filter let spurious weak low-kv anchors, e.g. lone "lord"/"israel"/"solomon" hits all pinning to kv=1 in 1ki6, form a longer flat chain that displaced the genuinely strong v1+v2+v3 anchors). The survivors are the confirmed anchors.
