...
| 25 | `A, 6-13` (left-margin Arabic numbers; NO in-text Geʽez numerals — see (g)). Body = **A5 TAIL** (`ልኤሆሙ ፡ ይትጋደሉ ፡ ወከነ ፡ ቃሎሙ ፡ ዐቢይ ።`, completing A5 across the p24 break) + **A6 (begins at `ወበቀለ`) through ~A13**; the last word `ወ…` is cut → continues p26. **Glyph-level text PENDING a convergence pass (see (h)); the margin-line draft was off by the A5 tail.** |
